...
import {
  Banknote, Users, Package, ShoppingCart, TrendingUp,
  AlertCircle, Warehouse, BarChart2, Activity, ClipboardList,
  ArrowDown, RefreshCw, Monitor, Globe, Store, Phone, Zap,
  TrendingDown, ExternalLink, ShoppingBag, Wifi, X, Star, CalendarClock,
} from "lucide-react";
import {
...
function ExpiryAlertPanel() {
  ...
  const { data: expData } = useQuery({
    ...
    queryFn: () => customFetch<{ items: ExpiringProduct[] }>("/api/reports/expired-inventory?alertDays=30"),
    refetchInterval: 60_000,
  });
  useEffect(() => {
    if (!expData) return;
    setItems((expData.items ?? []).filter(p => {
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-orange-500 mt-0.5">
              {expired.length > 0 && <span className="font-semibold">{expired.length} expired · </span>}
              {expiringSoon.length > 0 && <span>{expiringSoon.length} expiring within 30 days</span>}
            </p>
          </div>
        </div>
        <button onClick={() => setDismissed(true)} aria-label="Dismiss alert" className="text-orange-400 hover:text-orange-600 transition-colors mt-0.5">
          <X className="h-4 w-4" />
        </button>
      </CardHeader>
      <CardContent>
        <div className="grid gap-1.5 max-h-[200px] overflow-y-auto pr-1">
          {items.slice(0, 10).map(p => {
            ...
            return (
              <div key={p.id} className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl bg-white dark:bg-orange-950/20 border border-orange-100 dark:border-orange-900/30">
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold truncate">{p.name}</p>
                  <p className="text-[10px] text-muted-foreground">{p.stock} units · {GHS(p.stockValue)}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Badge className={`text-[10px] h-5 px-2 ${isExpired ? "bg-red-500 text-white" : daysLeft <= 7 ? "bg-orange-500 text-white" : "bg-amber-100 text-amber-700 border-0 dark:bg-amber-900/40 dark:text-amber-300"}`}>
          ...
          {items.length > 10 && (
            <p className="text-center text-xs text-muted-foreground py-1">+{items.length - 10} more — check Reports → Expired tab</p>
          )}
        </div>
      </CardContent>
    </Card>
...
function StockBadge({ stock, reorderPoint }: { stock: number; reorderPoint: number }) {
  ...
  if (stock <= reorderPoint)
    return <Badge variant="secondary" className="text-[10px] bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-200">Low</Badge>;
  return <Badge variant="outline" className="text-[10px] text-green-600 border-green-300">In Stock</Badge>;
}
...
export default function Dashboard() {
  ...
  const { data: revenueData, isLoading: revenueLoading } = useGetRevenueOverTime({ months: 6 }, {
    query: { refetchInterval: 60_000 },
  });
  const { data: recentSales, isLoading: recentSalesLoading } = useGetRecentSales({ limit: 5 }, {
    query: { refetchInterval: 60_000 },
  });
  const { data: topCustomers } = useGetTopCustomers({ limit: 5 }, { query: { refetchInterval: 60_000 } });
  const { data: topProductsData } = useGetTopProducts({ limit: 10 }, { query: { refetchInterval: 60_000 } });
  ...
  type PurchaseSummary = { totalOrders: number; totalSpend: number; received: number; pending: number; scope?: "all" | "own" };
  ...
  const { data: deadStockRaw } = useQuery({
    ...
    queryFn: () => customFetch<{ items: DeadStockItem[] }>("/api/reports/dead-stock?days=30&threshold=0"),
    refetchInterval: 60_000,
  });
  ...
  const { data: purchRaw } = useQuery({
    ...
    queryFn: () => customFetch<PurchaseSummary & Record<string, unknown>>(
      `/api/reports/purchases?startDate=${dateRange.start}&endDate=${dateRange.end}`
    ),
    refetchInterval: 60_000,
  });
  const purchSummary: PurchaseSummary | null = purchRaw && typeof purchRaw.totalOrders === "number"
    ? { totalOrders: purchRaw.totalOrders, totalSpend: purchRaw.totalSpend as number, received: purchRaw.received as number, pending: purchRaw.pending as number, scope: (purchRaw.scope as "all" | "own" | undefined) }
    : null;
  ...
  const criticalItems = allStockProducts.filter((p) => p.stock === 0 || p.stock <= Math.ceil(p.reorderPoint * 0.5)).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight truncate">Management Dashboard</h1>
            <AppVersionBadge />
          </div>
          <p className="text-sm sm:text-base text-muted-foreground">Overview of your sales &amp; inventory performance.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 bg-muted/50 rounded-xl px-3 py-2 border border-border/50 w-full sm:w-auto">
          <span className="text-xs text-muted-foreground font-medium whitespace-nowrap">Period:</span>
          <Input
      ...
      {summaryLoading ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <Card key={i} className="animate-pulse">
              ...
              <CardContent><div className="h-8 w-20 bg-muted rounded mb-2" /><div className="h-3 w-32 bg-muted rounded" /></CardContent>
            </Card>
          ))}
        </div>
      ) : summary ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium flex items-center gap-1.5 flex-wrap">
                ...
                {(summary as any).scope === "all" && (
                  <Badge variant="secondary" className="text-[9px] h-4 px-1.5">All users</Badge>
                )}
              </CardTitle>
              <span className="h-5 w-5 flex items-center justify-center rounded-full bg-muted text-muted-foreground font-bold text-sm">₵</span>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold" data-testid="kpi-total-revenue-value">{GHS(summary.totalRevenue)}</div>
              <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                <TrendingUp className="h-3 w-3 text-green-500" />
                <span className="text-green-500 font-medium">+{summary.revenueGrowth || 0}%</span> from last month
              </p>
            </CardContent>
          </Card>
          ...
          <Card data-testid="kpi-purchase-orders" data-scope={purchSummary?.scope ?? "own"}>
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="kpi-purchase-orders-value">{purchSummary?.totalOrders ?? 0}</div>
              <p className="text-xs text-muted-foreground mt-1" data-testid="kpi-purchase-orders-spend">
                {GHS(purchSummary?.totalSpend ?? 0)} spend
                {(purchSummary?.received ?? 0) > 0 && <> · <span className="text-green-600">{purchSummary!.received} received</span></>}
                {(purchSummary?.pending ?? 0) > 0 && <> · <span className="text-amber-600">{purchSummary!.pending} pending</span></>}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                <TrendingUp className="h-3 w-3 text-green-500" />
                <span className="text-green-500 font-medium">+{summary.salesGrowth || 0}%</span> from last month
              </p>
            </CardContent>
          </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {/* POS Quick Launch */}
        <Card className="relative overflow-hidden border-0 text-white" style={{ background: "linear-gradient(135deg, #7B2D42 0%, #1a2b5c 60%, #0D1B3E 100%)" }}>
          ...
          <CardHeader className="pb-2 relative z-10">
            ...
            <CardDescription className="text-white/60 text-xs flex items-center gap-1.5">
              <span
                ...
                style={posDevices.length > 0 ? { animation: "posConnPulse 1.8s ease-in-out infinite" } : {}}
              />
              {posDevices.length > 0
          ...
          <CardContent className="relative z-10 space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-white/10 rounded-xl p-3 text-center">
                ...
                <p className="text-xl font-bold">{channelData.find(c => c.channel === "pos")?.totalSales ?? 0}</p>
              </div>
              <div className="bg-white/10 rounded-xl p-3 text-center">
                <p className="text-white/60 text-xs mb-1">POS Revenue</p>
                <p className="text-sm font-bold">{GHS(channelData.find(c => c.channel === "pos")?.revenue ?? 0)}</p>
              </div>
            </div>
                ...
                {posDevices.map((device) => (
                  <div key={device.id} className="flex items-center gap-2 px-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[10px] text-white/50 capitalize">{device.deviceType.replace(/_/g, " ")}</span>
                    </div>
                    <button
                  ...
                  return (
                    <div key={c.channel} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          {CHANNEL_ICONS[c.channel] ?? <Store className="h-4 w-4 text-muted-foreground" />}
                          <span className="font-medium capitalize">{c.channel}</span>
                          <Badge variant="outline" className="text-[10px] h-4 px-1.5">{c.totalSales} sales</Badge>
                        </div>
              ...
              <div className="h-[280px]">
                <ResponsiveContainer width="100%" height={280} minWidth={0}>
                  <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={{ borderRadius: "8px", border: "1px solid hsl(var(--border))" }} formatter={(v: number) => [GHS(v), "Revenue"]} />
                    <Area type="monotone" dataKey="revenue" stroke="hsl(var(--primary))" strokeWidth={2} fillOpacity={1} fill="url(#revGrad)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          {(sale as unknown as { channel?: string }).channel && (sale as unknown as { channel?: string }).channel !== "online" && (
                            <Badge variant="outline" className="text-[9px] h-3.5 px-1 capitalize">{(sale as unknown as { channel?: string }).channel}</Badge>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            {allStockProducts.length > 0 ? (
              <div className="space-y-3">
                {allStockProducts.slice(0, 6).map((p) => {
                  ...
                  const barColor = isCritical ? "bg-red-500" : isLow ? "bg-amber-400" : "bg-emerald-500";
                  return (
                    <div key={p.id} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium truncate max-w-[180px]">{p.name}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-muted-foreground tabular-nums text-xs">{p.stock} units</span>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-950/30 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-red-700 dark:text-red-400">Critical Stock</span>
              </div>
            ...
            {lowStockData && lowStockData.total > 0 ? (
              <div className="space-y-2">
                {lowStockData.data.map((p) => (
                  <div key={p.id} className="flex items-center justify-between p-2.5 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/40">
                    <div className="flex-1 min-w-0">
        ...
        <Card>
          <CardHeader className="flex flex-row items-start justify-between">
            <div>
              ...
              <CardDescription>Products with zero sales in the last 30 days — cash flow risk</CardDescription>
            </div>
            <Button variant="outline" size="sm" className="rounded-full shrink-0" onClick={() => setLocation("/reports")}>
              Full Analysis
            </Button>
          </CardHeader>
          <CardContent>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {deadStockItems.map((item) => (
                <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl border bg-rose-50/50 dark:bg-rose-950/10 border-rose-100 dark:border-rose-900/30">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{item.stock} units · {GHS(item.stockValue)}</p>
                  </div>
          ...
          <CardContent>
            <div className="rounded-md border">
              ...
              {topCustomers.map((customer) => (
                <div key={customer.id} className="grid grid-cols-4 p-4 border-b last:border-0 items-center text-sm">
