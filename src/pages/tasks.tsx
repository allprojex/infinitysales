...
export default function Tasks() {
  ...
  const { data: projectsData } = useQuery<{ data: Project[] }>({
    queryKey: ["projects"],
    queryFn: () => customFetch("/api/projects?limit=100"),
  });
  ...
  const fetchTasks = async () => {
    ...
    await Promise.all(
      projects.map(async (p) => {
        try {
          const t = await customFetch<Task[]>(`/api/projects/${p.id}/tasks`);
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Dialog open={creating} onOpenChange={setCreating}>
            <DialogTrigger asChild>
              <Button className="rounded-full gap-2" disabled={!projects.length}>
                  ...
                  tasks.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {projectMap[t.projectId] ?? `Project #${t.projectId}`}
                      </TableCell>
                      <TableCell>
                        <Badge
                      ...
                      <TableCell>
                        <Badge
                          className={`${priorityColors[t.priority] ?? ""} border-0 text-[10px] capitalize`}
                        >
                          {t.priority}
                        </Badge>
                      </TableCell>
