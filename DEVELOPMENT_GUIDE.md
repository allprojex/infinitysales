...
- **Default admin bootstrap**: `ensureDefaultAdmin()` (`_auth-helpers.ts`) runs on **every** login POST request (idempotent per server process). If no profile exists for `admin@infinitysi.com`, it creates one with the **hardcoded password `Admin@123!`** and `must_change_password: true`. See [§29](#29-security-considerations).
...
- **Hardcoded default admin bootstrap.** `_auth-helpers.ts`'s `ensureDefaultAdmin()` runs on every login attempt and will auto-create `admin@infinitysi.com` with the **hardcoded password `Admin@123!`** if that account doesn't exist yet. Mitigated by `must_change_password: true`, but the plaintext credential lives in source control — a fresh/reset environment is one login POST away from having a known-password admin account until someone changes it.
