...
- **Default admin bootstrap is opt-in and unlogged.** `_auth-helpers.ts`'s `ensureDefaultAdmin()` runs on every login attempt but only creates an account if both `DEFAULT_ADMIN_BOOTSTRAP_EMAIL` and `DEFAULT_ADMIN_BOOTSTRAP_PASSWORD` are explicitly set in the server environment; it is a silent no-op otherwise, and never modifies an existing account. Neither value is ever written to logs. No credential is hardcoded in source control anymore. **Deployed** (commit `4d5caef`, 2026-07-22) — post-deploy verification on the production VPS confirmed both env vars are absent there, a real login only produced the fixed log line `[bootstrap admin] Skipped: DEFAULT_ADMIN_BOOTSTRAP_EMAIL/DEFAULT_ADMIN_BOOTSTRAP_PASSWORD not configured.` (no values), no account was created, and both existing admin accounts (`admin@infinitysi.com`, `infinitytechub@outlook.com`) are unchanged.
