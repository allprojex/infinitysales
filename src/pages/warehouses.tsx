...
function useWarehouses() {
  return useQuery<WarehouseRow[]>({
    queryKey: ["warehouses"],
    queryFn: () => customFetch("/api/warehouses"),
  });
}

function useWarehouseStock(id: number | null, categoryId: string) {
  return useQuery<StockRow[]>({
    queryKey: ["warehouse-stock", id, categoryId],
    queryFn: () =>
      customFetch(
        `/api/warehouses/${id}/stock${categoryId === "all" ? "" : `?categoryId=${categoryId}`}`,
      ),
    enabled: id !== null,
  });
}
...
function WarehouseForm({
  ...
  return (
    <div className="space-y-4 pt-2">
      <div>
        <label className="text-sm font-medium block mb-1">Warehouse Name *</label>
        <Input
          id="warehouse-name"
          name="name"
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button
          ...
          onClick={() => onSave({ name, location, address, isDefault })}
        >
...
export default function Warehouses() {
  ...
  const { data: stockRows = [], isLoading: stockLoading } = useWarehouseStock(
    viewingId,
    categoryFilter,
  );
  const { data: categoryResponse } = useQuery<{ data: Array<{ id: string; name: string }> }>({
    queryKey: ["product-categories", "warehouse-filter"],
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, ...d }: { id: number } & object) =>
      customFetch(`/api/warehouses/${id}`, {
        method: "PUT",
        body: JSON.stringify(d),
        headers: { "Content-Type": "application/json" },
      }),
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Multi-warehouse inventory management and stock tracking.
          </p>
        </div>
      ...
      >
        <DialogContent className="sm:max-w-[440px]">
          ...
          {editingWh && (
            <WarehouseForm
              initial={editingWh}
              onSave={(d) => updateMut.mutate({ id: editingWh.id, ...d })}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the warehouse and its stock assignments permanently.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {isLoading ? (
        ...
        <div className="bg-card rounded-3xl border border-dashed border-muted-foreground/20 p-12 text-center">
          ...
          <p className="text-muted-foreground mb-6">
            Add your first warehouse to start tracking multi-location stock.
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {warehouses.map((wh) => (
            <Card
              key={wh.id}
              className="border-transparent shadow-sm hover:shadow-md transition-shadow"
            >
              <CardHeader className="pb-2 flex flex-row items-start justify-between gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => setEditingWh(wh)}>
                      <Pencil className="h-4 w-4 mr-2" />
                      Edit
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() => setViewingId(viewingId === wh.id ? null : wh.id)}
                    >
          ...
          <CardContent>
            {stockLoading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-6">
                No stock assigned to this warehouse yet.
              </p>
            ) : (
              <div className="rounded-md border">
                ...
                {stockRows.map((r) => (
                  <div
                    key={r.product.id}
                    className="grid grid-cols-3 p-3 border-b last:border-0 text-sm items-center"
                  >
