...
import {
  ClipboardCheck, Plus, RefreshCw, Play, CheckCircle2, Trash2,
  ArrowLeft, Search, AlertTriangle, ChevronDown, ChevronUp, TrendingUp, TrendingDown,
} from "lucide-react";
...
const fmt = (d?: string | null) =>
  d ? new Date(d).toLocaleDateString("en-GH", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "—";

/* ── Types ──────────────────────────────────────────────── */
interface StockTake {
  id: number; title: string; warehouse_name: string | null;
  status: string; total_items: number; counted_items: number;
  total_variance: number; total_variance_value: string;
  notes: string | null; started_at: string | null; completed_at: string | null;
  created_by: string | null; created_at: string;
}
interface StockTakeItem {
  id: number; stock_take_id: number; product_id: number;
  product_name: string; product_sku: string | null; product_category: string | null;
  expected_qty: number; counted_qty: number | null;
  variance: number | null; unit_price: string; variance_value: string | null;
  notes: string | null; counted_at: string | null; counted_by: string | null;
}
interface StockTakeDetail extends StockTake { items: StockTakeItem[]; }
  ...
  return (
    <TableRow className={isCounted && variance !== 0 ? "bg-amber-500/5" : ""}>
      <TableCell>
        ...
        <div className="text-[10px] text-muted-foreground">{item.product_sku ?? ""} {item.product_category ? `· ${item.product_category}` : ""}</div>
      </TableCell>
      ...
      <TableCell className="text-center">
        {variance !== null ? (
          <span className={`font-semibold text-sm inline-flex items-center gap-0.5 ${
            variance > 0 ? "text-emerald-400" : variance < 0 ? "text-red-400" : "text-muted-foreground"
          }`}>
      ...
      <TableCell className="text-right text-xs text-muted-foreground hidden lg:table-cell">
        {variance !== null && variance !== 0
          ? <span className={variance > 0 ? "text-emerald-400" : "text-red-400"}>{GHS(Math.abs(Number(item.variance_value ?? 0)))}</span>
          : "—"}
      </TableCell>
      <TableCell className="hidden md:table-cell">
        {editing ? (
          <Input className="h-7 text-xs w-32" value={notes}
            onChange={e => setNotes(e.target.value)} placeholder="Note…" />
        ) : (
        ...
        <TableCell>
          {editing ? (
            <div className="flex gap-1">
              ...
              <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setEditing(false)}>✕</Button>
            </div>
          ) : (
            <Button variant="ghost" size="sm" className="h-7 text-xs"
              onClick={() => { setQty(item.counted_qty !== null ? String(item.counted_qty) : ""); setNotes(item.notes ?? ""); setEditing(true); }}>
...
function StockTakeDetail({
  ...
  const updateItem = useMutation({
    mutationFn: ({ itemId, countedQty, notes }: { itemId: number; countedQty: number | null; notes: string }) =>
  ...
  const completeTake = useMutation({
    mutationFn: () => customFetch(`/api/stock-takes/${takeId}/complete`, {
      method: "POST",
      body: JSON.stringify({ commitAdjustments: commitAdj }),
    }),
    onSuccess: (data: any) => {
      toast({ title: data.committed ? "Completed & stock adjusted" : "Completed (no adjustments)" });
      qc.invalidateQueries({ queryKey: ["stock-take", takeId] });
      qc.invalidateQueries({ queryKey: ["stock-takes"] });
  ...
  if (filterState === "variance")  filtered = filtered.filter(i => i.counted_qty !== null && i.counted_qty !== i.expected_qty);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <Badge variant="outline" className={`capitalize text-xs ${STATUS_COLORS[take.status] ?? ""}`}>
          {take.status.replace("_", " ")}
        </Badge>
        {isAdmin && take.status === "draft" && (
          <Button size="sm" className="gap-1.5" onClick={() => startTake.mutate()} disabled={startTake.isPending}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="col-span-2 sm:col-span-2 border-blue-500/20 bg-blue-500/5">
          <CardContent className="pt-4 pb-3">
            ...
            <p className="text-[10px] text-muted-foreground mt-1">{countedCount} of {take.total_items} items counted</p>
          </CardContent>
        </Card>
        ...
        <Card className={`${totalVarVal < 0 ? "border-red-500/20 bg-red-500/5" : "border-emerald-500/20 bg-emerald-500/5"}`}>
          <CardContent className="pt-4 pb-3">
            <p className="text-xs text-muted-foreground mb-1">Value Variance</p>
            <p className={`text-xl font-bold ${totalVarVal < 0 ? "text-red-400" : "text-emerald-400"}`}>
              {totalVarVal >= 0 ? "+" : ""}{GHS(totalVarVal)}
            </p>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative flex-1 min-w-40">
          ...
          <Input className="h-8 text-xs pl-8" placeholder="Search product…"
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-3">
              <span className="block">
                ...
                {variances.length > 0 && ` ${variances.length} variances found (${GHS(Math.abs(totalVarVal))}).`}
              </span>
              <label className="flex items-center gap-2 cursor-pointer mt-3">
                <input type="checkbox" checked={commitAdj}
...
export default function StockTake() {
  ...
  const { data: takes, isLoading, refetch } = useQuery<StockTake[]>({
    queryKey: ["stock-takes"],
    queryFn: () => customFetch<any>("/api/stock-takes").then((d) => (Array.isArray(d) ? d : d?.data ?? [])),
  });
  ...
  const createTake = useMutation({
    mutationFn: (body: object) => customFetch("/api/stock-takes", { method: "POST", body: JSON.stringify(body) }),
    onSuccess: (data: any) => {
    ...
    <div className="p-4 sm:p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Physical count sessions — verify and reconcile inventory</p>
          </div>
        </div>
        ...
        <Card className="border-blue-500/30 bg-blue-500/5">
          <CardContent className="pt-4 pb-3">
            <p className="text-xs font-semibold text-blue-300 mb-2 flex items-center gap-1.5">
              <AlertTriangle className="h-3.5 w-3.5" />Active Count Sessions
            </p>
              ...
              {active.map(t => (
                <div key={t.id} className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-xs text-muted-foreground ml-2">{t.warehouse_name ?? "All"}</span>
                  </div>
      ...
      <Card>
        <Table>
          ...
          <TableBody>
            ...
            {rows.map(t => {
              const progress = t.total_items > 0 ? Math.round((t.counted_items / t.total_items) * 100) : 0;
              return (
                <TableRow key={t.id} className="cursor-pointer hover:bg-muted/30"
                  onClick={() => setActiveTakeId(t.id)}>
                  <TableCell>
                    <div className="font-medium text-sm">{t.title}</div>
                    {t.created_by && <div className="text-[10px] text-muted-foreground">by {t.created_by}</div>}
                  </TableCell>
                  <TableCell className="text-sm hidden sm:table-cell">
                    {t.warehouse_name ?? <span className="text-muted-foreground">All Locations</span>}
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-muted-foreground w-8 shrink-0">{progress}%</span>
                    </div>
                    <p className="text-[10px] text-muted-foreground">{t.counted_items}/{t.total_items}</p>
                  </TableCell>
                  <TableCell className="text-right hidden md:table-cell">
                    {t.status === "completed" && t.total_variance !== 0 ? (
                      <span className={`text-sm font-semibold ${Number(t.total_variance) > 0 ? "text-emerald-400" : "text-red-400"}`}>
                        {Number(t.total_variance) > 0 ? "+" : ""}{t.total_variance} units
                        <div className="text-[10px] font-normal">{GHS(Math.abs(Number(t.total_variance_value)))}</div>
                      </span>
                    ) : <span className="text-muted-foreground text-xs">—</span>}
                  </TableCell>
                  ...
                  <TableCell onClick={e => e.stopPropagation()}>
                    <Badge variant="outline" className={`text-[10px] capitalize ${STATUS_COLORS[t.status] ?? ""}`}>
                      {t.status.replace("_", " ")}
                    </Badge>
                  </TableCell>
                  <TableCell onClick={e => e.stopPropagation()}>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="sm" className="h-7 text-xs"
                        onClick={() => setActiveTakeId(t.id)}>Open</Button>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>New Stock Take Session</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <div>
              ...
              <Textarea className="text-xs" rows={2} value={form.notes}
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={open => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>All count data will be permanently deleted.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
