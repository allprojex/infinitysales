...
export default function BankReconciliation() {
  ...
  const accounts: BankAccount[] = Array.isArray(accountsResp)
    ? accountsResp
    : (accountsResp?.data ?? []);
  ...
  const saveAcctMutation = useMutation({
    mutationFn: () => {
      ...
      return editAcct
        ? customFetch(`/api/bank-accounts/${editAcct.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          })
        : customFetch("/api/bank-accounts", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          });
    ...
    <div className="flex h-full overflow-hidden">
        ...
        {summary && (
          <div className="px-3 py-2 grid grid-cols-2 gap-1.5 border-b">
            ...
            <div className="rounded bg-muted/40 px-2 py-1">
              ...
              <p
                className={`text-xs font-bold ${summary.unreconciled_txns > 0 ? "text-amber-400" : "text-muted-foreground"}`}
              >
          ...
          {accounts.map((a) => (
            ...
            >
              <div className="flex items-start gap-2">
                <div
                  className={`h-7 w-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${a.is_active ? "bg-primary/20" : "bg-muted"}`}
                >
                  <Landmark
                    className={`h-3.5 w-3.5 ${a.is_active ? "text-primary" : "text-muted-foreground"}`}
                  />
                </div>
        ...
        {!selected ? (
          ...
          <>
            ...
            <div className="border-b px-5 py-3 flex items-center justify-between shrink-0">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className={`text-[10px] ${detail.account.is_active ? "text-emerald-400 border-emerald-500/30" : "text-muted-foreground"}`}
                  >
                ...
                <p className="text-xs text-muted-foreground font-mono">
                  ...
                  {ACCOUNT_TYPES.find((t) => t.value === detail.account.account_type)?.label}
                </p>
              </div>
              <div className="flex gap-2 shrink-0">
                ...
                <Button
                  ...
                  className="gap-1.5"
                  onClick={() => {
                    setEditAcct(detail.account);
                    setAcctForm({
                      name: detail.account.name,
                      bankName: detail.account.bank_name,
                      accountNumber: detail.account.account_number,
                      accountType: detail.account.account_type,
                      openingBalance: detail.account.opening_balance,
            ...
            <div className="px-5 py-3 grid grid-cols-2 sm:grid-cols-4 gap-3 border-b shrink-0">
              <Card className="border-primary/20 bg-primary/5">
                <CardContent className="pt-3 pb-2">
                  ...
                  <p className="text-xl font-bold text-primary">
                    {ghc(detail.account.current_balance)}
                  </p>
                  <p className="text-[10px] text-muted-foreground">
                    Opening: {ghc(detail.account.opening_balance)}
                  </p>
                </CardContent>
              </Card>
              <Card className="border-emerald-500/20 bg-emerald-500/5">
                <CardContent className="pt-3 pb-2">
                  ...
                  <p className="text-xl font-bold text-emerald-400">
                    {ghc(detail.stats.total_credits)}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card
                className={
                  detail.stats.unreconciled_txns > 0
                ...
                <CardContent className="pt-3 pb-2">
                  ...
                  <p className="text-xl font-bold">
                    ...
                    <span className="text-muted-foreground text-base">
                      {" "}
                      / {detail.stats.total_txns}
                    </span>
                  </p>
                  {unreconciledBalance !== 0 && (
                    <p
                      className={`text-[10px] ${unreconciledBalance > 0 ? "text-emerald-400" : "text-red-400"}`}
                    >
                      {unreconciledBalance > 0 ? "+" : ""}
                      {ghc(unreconciledBalance)} pending
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              <Tabs value={filterTab} onValueChange={setFilterTab}>
                <TabsList className="h-7">
                  <TabsTrigger value="all" className="text-xs h-6">
                    All ({detail.stats.total_txns})
                  </TabsTrigger>
                  <TabsTrigger value="unreconciled" className="text-xs h-6">
                    Unreconciled
                    {detail.stats.unreconciled_txns > 0 && (
                      <span className="ml-1 bg-amber-500 text-white rounded-full text-[9px] px-1">
                        {detail.stats.unreconciled_txns}
                      </span>
                    )}
                  </TabsTrigger>
                  <TabsTrigger value="reconciled" className="text-xs h-6">
                    Reconciled ({detail.stats.reconciled_txns})
                  </TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {filteredTxns.map((t) => (
                    <TableRow
                      key={t.id}
                      className={`text-xs group ${t.reconciled ? "opacity-60" : ""}`}
                    >
                      <TableCell className="pl-5">
                        <Checkbox
                          checked={t.reconciled}
                          onCheckedChange={(v) =>
                            reconcileMutation.mutate({ txnId: t.id, reconciled: !!v })
                          }
                        />
                      </TableCell>
                      ...
                      <TableCell
                        className={`text-right font-mono font-semibold ${t.type === "credit" ? "text-emerald-400" : "text-red-400"}`}
                      >
                        {t.type === "credit" ? "+" : "-"}
                        {ghc(t.amount)}
                      </TableCell>
      ...
      <Dialog open={acctDlg} onOpenChange={setAcctDlg}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{editAcct ? "Edit Account" : "Add Bank Account"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setAcctForm((f) => ({ ...f, bankName: e.target.value }))}
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setAcctForm((f) => ({ ...f, accountNumber: e.target.value }))}
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setAcctForm((f) => ({ ...f, accountType: v }))}
              >
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAcctDlg(false)}>
              Cancel
            </Button>
            <Button
              onClick={() => saveAcctMutation.mutate()}
              disabled={
                !acctForm.name ||
                !acctForm.bankName ||
                !acctForm.accountNumber ||
                saveAcctMutation.isPending
              }
            >
      ...
      <Dialog open={txnDlg} onOpenChange={setTxnDlg}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Add Bank Entry</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-1">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setTxnForm((f) => ({ ...f, txnDate: e.target.value }))}
                autoFocus
              />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Salary payment, Sales deposit…"
              />
            </div>
            <div className="space-y-1.5">
      ...
      <AlertDialog open={!!delAcctDlg} onOpenChange={(o) => !o && setDelAcctDlg(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete <strong>{delAcctDlg?.name}</strong>? Accounts with transactions cannot be
      ...
      <AlertDialog open={!!delTxnId} onOpenChange={(o) => !o && setDelTxnId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Permanently delete this entry? The account balance will be recalculated.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
