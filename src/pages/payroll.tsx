...
export default function Payroll() {
  ...
  const createRun = useMutation({
    mutationFn: (body: object) =>
      customFetch("/api/payroll", { method: "POST", body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const updateRun = useMutation({
    mutationFn: ({ id, body }: { id: string; body: object }) =>
      customFetch(`/api/payroll/${id}`, { method: "PUT", body: JSON.stringify(body) }),
    onSuccess: () => {
      ...
      <div className="grid grid-cols-3 gap-2">
        <div>
          ...
          <Input
            ...
            min="0"
            value={form.ssnit}
            onChange={(e) => setForm((f) => ({ ...f, ssnit: e.target.value }))}
          />
        </div>
        <div>
          <label className="text-xs text-muted-foreground font-medium mb-1 block">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Manage employee payroll runs and payslips
            </p>
          </div>
        </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => createRun.mutate(buildBody())}
              disabled={createRun.isPending || !form.employeeId || !form.month}
            >
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select
                value={form.employeeId ? (editRun?.status ?? "draft") : "draft"}
                onValueChange={(v) => setEditRun((r) => (r ? { ...r, status: v } : null))}
              >
                <SelectTrigger className="h-8 text-xs">
                  <SelectValue />
                </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() =>
                ...
                updateRun.mutate({
                  id: editRun.id,
                  body: { ...buildBody(), status: editRun.status },
                })
              }
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => deleteId !== null && deleteRun.mutate(deleteId)}
