...
import {
  Printer, AlertTriangle, Loader2, Usb, Bluetooth, Network, Plus,
  RefreshCw, Eye, Tag, QrCode, Hash, Package, BarChart, Clock,
...
const LABEL_TYPES: { value: LabelType; label: string; desc: string; icon: React.ReactNode }[] = [
  { value: "barcode", label: "Barcode Label", desc: "Product barcode with name & price", icon: <BarChart className="h-4 w-4" /> },
  { value: "shelf", label: "Shelf Label", desc: "Retail shelf display tag", icon: <Tag className="h-4 w-4" /> },
  { value: "qr", label: "QR Code Label", desc: "QR code with product details", icon: <QrCode className="h-4 w-4" /> },
  { value: "sku", label: "SKU Label", desc: "Internal SKU identifier label", icon: <Hash className="h-4 w-4" /> },
  { value: "inventory", label: "Inventory Label", desc: "Warehouse bin / stock label", icon: <Package className="h-4 w-4" /> },
  { value: "price_tag", label: "Price Tag", desc: "Standalone GHS price sticker", icon: <FileText className="h-4 w-4" /> },
];
...
function LabelPreview({ type, form }: { type: LabelType; form: Record<string, string> }) {
  ...
  const ghsPrice = new Intl.NumberFormat("en-GH", { style: "currency", currency: "GHS" }).format(price);

  const baseClass = "border-2 border-dashed border-gray-300 rounded-xl bg-white text-black p-4 max-w-[320px] mx-auto font-sans text-[11px] select-none";

  if (type === "barcode") return (
    <div className={baseClass}>
      <p className="font-bold text-sm text-center mb-1 leading-tight">{form.productName || "Product Name"}</p>
      <p className="text-center text-gray-500 mb-2">{form.sku || "SKU-000"} {form.category ? `· ${form.category}` : ""}</p>
      ...
      {form.promoText && <p className="text-center text-red-600 text-[10px] mt-0.5">{form.promoText}</p>}
    </div>
  );
  ...
  if (type === "inventory") return (
    <div className={baseClass}>
      ...
      {form.promoText && <p className="text-center text-gray-600 text-[9px] mt-1">{form.promoText}</p>}
    </div>
  );
...
export default function LabelPrinterPage() {
  ...
  const [form, setForm] = useState({
    productName: "", sku: "", barcode: "", price: "", stock: "",
    category: "", promoText: "",
  });
  ...
  const logJobMut = useMutation({
    mutationFn: (data: object) => customFetch("/api/label-printer/jobs", {
  ...
  async function detectPrinters() {
    ...
    if ("usb" in navigator) {
      try {
        ...
        for (const d of devices) {
          if (d.productName) {
            found.push({ id: d.serialNumber ?? d.productName, name: d.productName, type: "usb", status: "ready" });
          }
        }
      } catch { /* ignore */ }
    }
    ...
    if ("bluetooth" in navigator) {
      try {
        ...
        for (const d of devices) {
          found.push({ id: d.id, name: d.name ?? "Bluetooth Printer", type: "bluetooth", status: "ready" });
        }
      } catch { /* ignore */ }
    }

    if (found.length === 0) {
      found.push({ id: "system-default", name: "System Default Printer", type: "network", status: "ready" });
    }
  ...
  async function scanNetworkPrinters() {
    ...
    try {
      ...
      if (result.length === 0) {
        toast({ title: "No network printers found", description: `No printers responding on ${networkScanSubnet}.0/24 port 9100.` });
      } else {
        setPrinters(p => {
          const existingIds = new Set(p.map(x => x.id));
      ...
      toast({ title: "Network scan failed", description: "Could not reach the server scanner.", variant: "destructive" });
    }
    setScanning(false);
  }
  ...
  function addNetworkPrinterManual() {
    ...
    if (!ip) return;
    const np: Printer = { id: `net-${ip}`, name: `Network Printer (${ip})`, type: "network", address: ip, status: "ready" };
    setPrinters(p => [...p, np]);
  ...
  function handlePrint() {
    if (!selectedPrinter) {
      toast({ title: "Select a printer first", variant: "destructive" }); return;
    }
    if (!form.productName.trim()) {
      toast({ title: "Enter a product name", variant: "destructive" }); return;
    }
    ...
    logJobMut.mutate({
      labelType, printerName: selectedPrinter.name,
      printerType: selectedPrinter.type,
      productName: form.productName, sku: form.sku || undefined,
      barcode: form.barcode || undefined, price: form.price ? Number(form.price) : undefined,
      stock: form.stock ? Number(form.stock) : undefined,
      category: form.category || undefined, promoText: form.promoText || undefined, copies,
    });

    toast({ title: "Sent to printer", description: `${copies}× ${labelType} label for ${form.productName}` });
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="border border-white/10 bg-white/5 rounded-2xl">
            ...
            <CardContent className="space-y-3">
              <div className="flex gap-2 flex-wrap">
                <Button variant="outline" className="gap-2 rounded-[20px] text-sm" onClick={detectPrinters} disabled={detecting}>
                  {detecting ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                  {detecting ? "Detecting…" : "Detect USB/BT"}
                </Button>
                <Button variant="outline" className="gap-2 rounded-[20px] text-sm"
                ...
                <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 space-y-1">
                      <label className="text-xs text-muted-foreground">Subnet (first 3 octets)</label>
                      <Input
                    ...
                    <div className="flex items-end">
                      <Button size="sm" className="rounded-[14px] gap-1.5" onClick={scanNetworkPrinters} disabled={scanning}>
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <Button size="sm" variant="outline" className="rounded-[14px] shrink-0"
                      onClick={addNetworkPrinterManual} disabled={!networkIpInput.trim()}>
                      <Plus className="h-3.5 w-3.5 mr-1" /> Add
                    </Button>
                  </div>
                  <Button size="sm" variant="ghost" className="w-full text-xs" onClick={() => setShowNetworkDialog(false)}>
                    Close
                  </Button>
                </div>
              )}
              ...
              <div className="flex gap-2 flex-wrap">
                {("usb" in navigator) && (
                  <Button variant="outline" className="gap-2 rounded-[20px] text-sm" onClick={async () => {
                    try {
                      const d = await (navigator as any).usb.requestDevice({ filters: [] });
                      const p: Printer = { id: d.serialNumber ?? d.productName, name: d.productName ?? "USB Printer", type: "usb", status: "ready" };
                      setPrinters(prev => [...prev.filter(x => x.id !== p.id), p]);
                      setSelectedPrinter(p);
                    } catch { toast({ title: "No USB printer selected", variant: "destructive" }); }
                  }}>
              ...
              {printers.length === 0 ? (
                <p className="text-sm text-muted-foreground py-2">
                  No printers found. Click "Detect Printers" or add a network printer by IP address. Your OS printer drivers must be installed.
                </p>
              ) : (
                <div className="space-y-1.5">
                  {printers.map(p => (
                    <button key={p.id} type="button"
                      ...
                      {p.type === "usb" ? <Usb className="h-4 w-4 text-sky-400 shrink-0" /> :
                        p.type === "bluetooth" ? <Bluetooth className="h-4 w-4 text-violet-400 shrink-0" /> :
                          <Network className="h-4 w-4 text-emerald-400 shrink-0" />}
                      <div className="flex-1 min-w-0">
                        <p className="font-medium truncate">{p.name}</p>
                        <p className="text-xs text-muted-foreground capitalize">{p.type}{p.address ? ` · ${p.address}` : ""}</p>
                      </div>
                      <span className={`h-2 w-2 rounded-full shrink-0 ${p.status === "ready" ? "bg-emerald-400" : "bg-zinc-500"}`} />
                    </button>
                  ))}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {LABEL_TYPES.map(lt => (
                  <button key={lt.value} type="button"
                    ...
                    <div className="flex items-center gap-1.5 mb-0.5 font-medium">{lt.icon}{lt.label}</div>
          ...
          <Card className="border border-white/10 bg-white/5 rounded-2xl">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <Label>Barcode</Label>
                  <Input className="rounded-[20px]" placeholder="6001068012345"
              ...
              <div className="space-y-1">
                ...
                <Input className="rounded-[20px]" placeholder="e.g. 10% OFF – Ends Friday"
                  value={form.promoText} onChange={e => setForm(f => ({ ...f, promoText: e.target.value }))} />
              </div>
              <div className="space-y-1">
                <Label>Copies</Label>
                <Input type="number" min={1} max={100} className="rounded-[20px] w-24"
                  value={copies} onChange={e => setCopies(Math.max(1, Math.min(100, Number(e.target.value))))} />
              </div>
            </CardContent>
          </Card>

          <Button onClick={handlePrint} className="w-full gap-2 rounded-[20px] bg-violet-600 hover:bg-violet-700 text-white h-11"
            disabled={!form.productName.trim()}>
            <Printer className="h-5 w-5" /> Print {copies > 1 ? `${copies}×` : ""} {LABEL_TYPES.find(l => l.value === labelType)?.label}
          </Button>
        </div>
        ...
        <div className="space-y-4">
          <Card className="border border-white/10 bg-white/5 rounded-2xl">
            ...
            <CardContent>
              <div className="bg-gray-100 rounded-xl p-6 min-h-[200px] flex items-center justify-center" ref={printRef}>
                    ...
                    <TableBody>
                      {jobs.map(j => (
                        <TableRow key={j.id} className="border-white/10 text-sm">
                          <TableCell className="font-medium max-w-[100px] truncate">{j.productName}</TableCell>
                          <TableCell>
                            <Badge variant="outline" className="text-[10px] capitalize">
                              {j.labelType.replace(/_/g, " ")}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-muted-foreground text-xs truncate max-w-[80px]">{j.printerName}</TableCell>
                          <TableCell>{j.copies}</TableCell>
                          <TableCell className="text-muted-foreground text-xs whitespace-nowrap">
                            {new Date(j.timestamp).toLocaleString("en-GH", { dateStyle: "short", timeStyle: "short" })}
                          </TableCell>
                          <TableCell className="text-muted-foreground text-xs">{j.initiatedBy}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
