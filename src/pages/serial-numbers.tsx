...
function useSNs(productId: string, status: string) {
  return useQuery<{ data: SN[]; total: number }>({
    ...
    queryFn: () =>
      customFetch(
        `/api/serial-numbers?limit=100${productId !== "all" ? `&productId=${productId}` : ""}${status !== "all" ? `&status=${status}` : ""}`,
      ),
  });
}
...
export default function SerialNumbers() {
  ...
  const { data: categoryResponse } = useQuery<{ data: Array<{ id: string; name: string }> }>({
    queryKey: ["product-categories", "serial-number-filter"],
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewSN({ productId: "", serialNumber: "", status: "available", notes: "" });
      invalidate();
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Track individual unit serial numbers across your product inventory.
          </p>
        </div>
            ...
            <div className="space-y-4 pt-2">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    setNewSN((s) => ({ ...s, productId: "" }));
                  }}
                >
                  <SelectTrigger className="rounded-[20px] mb-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Select
                  value={newSN.productId}
                  onValueChange={(v) => setNewSN((s) => ({ ...s, productId: v }))}
                >
              ...
              <div>
                ...
                <Input
                  ...
                  value={newSN.serialNumber}
                  onChange={(e) => setNewSN((s) => ({ ...s, serialNumber: e.target.value }))}
              ...
              <div>
                ...
                <Select
                  value={newSN.status}
                  onValueChange={(v) => setNewSN((s) => ({ ...s, status: v }))}
                >
                  <SelectTrigger className="rounded-[20px]">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Textarea
                  ...
                  value={newSN.notes}
                  onChange={(e) => setNewSN((s) => ({ ...s, notes: e.target.value }))}
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative w-[220px]">
          ...
          <Input
            id="sn-search"
            name="search"
            placeholder="Search S/N or product…"
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this serial number record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {isLoading ? (
        ...
        <div className="bg-card rounded-3xl border border-dashed border-muted-foreground/20 p-12 text-center">
          ...
          <p className="text-muted-foreground mb-6 max-w-sm mx-auto">
            Register serial numbers to track individual units — useful for electronics, appliances,
        ...
        <div className="bg-card rounded-2xl border overflow-hidden shadow-sm">
          ...
          {sns.map((sn) => (
            <div
              key={sn.id}
              className="grid grid-cols-5 px-4 py-3 border-b last:border-0 items-center text-sm hover:bg-muted/10 transition-colors"
            >
              ...
              <div>
                <Badge className={`text-[10px] border-0 ${statusColor(sn.status)}`}>
                  {sn.status}
                </Badge>
              </div>
      ...
      <p className="text-xs text-muted-foreground text-right">
        {data?.total ?? 0} serial number{(data?.total ?? 0) !== 1 ? "s" : ""} total
      </p>
    </div>
  );
}
