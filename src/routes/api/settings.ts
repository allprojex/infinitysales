  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const own = data?.data && typeof data.data === "object" && !Array.isArray(data.data)
        ...
        try {
          ...
          return errorJson(500, permissionError instanceof Error ? permissionError.message : "Permission load failed");
        }
      },
      PUT: async ({ request }) => {
        const { user, response } = await requireUser(request);
