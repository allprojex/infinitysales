...
export default function Notifications() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<Notif[] | { data: Notif[] } | { items: Notif[] }>({
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="h-8">
            ...
            <TabsTrigger value="unread" className="text-xs px-3 h-7 gap-1">
              Unread
              {(summary?.unread ?? 0) > 0 && (
                <span className="text-[10px] bg-red-500 text-white rounded-full px-1">
                  {summary!.unread}
                </span>
              )}
            </TabsTrigger>
        ...
        {!isLoading && rows.length === 0 && (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Refresh to check for new alerts</p>
            </CardContent>
          </Card>
        )}
        {rows.map((n) => {
          ...
          return (
            <div
              ...
              className={`flex items-start gap-3 p-3 rounded-lg border transition-all
                ${
                  n.is_read ? "bg-card border-border opacity-60" : "bg-card border-border shadow-sm"
                }`}
            >
              {/* Severity dot */}
              <div
                className={`mt-0.5 h-7 w-7 rounded-lg flex items-center justify-center shrink-0 ${sev.bg}`}
              >
