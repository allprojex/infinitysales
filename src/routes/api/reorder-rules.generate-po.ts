    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const productIds = Array.from(
          new Set(ruleList.map((r: any) => r.product_id).filter(Boolean)),
        );
        const { data: products } = await sb
          .from("products")
          .select("id,name,sku,stock,cost,supplier_id")
          .eq("user_id", auth.user.id)
        ...
        for (const r of ruleList as any[]) {
          ...
          const reorderQty = Number(
            r.reorder_qty ?? r.reorder_quantity ?? Math.max(minLevel * 2 - currentStock, 1),
          );
