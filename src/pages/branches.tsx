...
export default function Branches() {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return editTarget
        ? customFetch(`/api/branches/${editTarget.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          })
        : customFetch("/api/branches", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          });
          ...
          {branches.map((b) => (
            ...
            >
              <div className="flex items-start gap-2">
                <div
                  className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${b.is_active ? "bg-emerald-500/20" : "bg-muted"}`}
                >
                  <Building2
                    className={`h-4 w-4 ${b.is_active ? "text-emerald-400" : "text-muted-foreground"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1">
                    ...
                    {b.is_default && (
                      <Star className="h-2.5 w-2.5 text-amber-400 shrink-0" fill="currentColor" />
                    )}
                  </div>
                  ...
                  <Badge
                    variant="outline"
                    className={`mt-1 text-[9px] py-0 ${b.is_active ? "text-emerald-400 border-emerald-500/30" : "text-muted-foreground"}`}
                  >
            ...
            <div className="border-b px-5 py-3 flex items-center justify-between shrink-0">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className={`text-[10px] ${detail.branch.is_active ? "text-emerald-400 border-emerald-500/30" : "text-muted-foreground"}`}
                  >
              ...
              <div className="flex gap-2 shrink-0">
                <Button
                  ...
                  variant="outline"
                  className="gap-1.5"
                  onClick={() => openEdit(detail.branch)}
                >
                ...
                {!detail.branch.is_default && (
                  <Button
                    ...
                    onClick={() => setDelTarget(detail.branch)}
                  >
                  ...
                  <Card className="border-primary/20 bg-primary/5">
                    <CardContent className="pt-3 pb-2">
                      ...
                      <p className="text-lg font-bold text-primary">
                        {ghc(detail.performance.revenue_30d)}
                      </p>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="pt-3 pb-2">
                      ...
                      <p className="text-[10px] text-amber-400">
                        {detail.performance.pending_sales} pending
                      </p>
                    </CardContent>
                  </Card>
                  <Card>
                    ...
                    <CardContent className="pt-3 pb-2">
                      ...
                      <div className="flex gap-2 mt-1 flex-wrap">
                        <span className="text-[10px] text-emerald-400">
                          Cash {detail.performance.cash_sales}
                        </span>
                        <span className="text-[10px] text-blue-400">
                          MoMo {detail.performance.momo_sales}
                        </span>
                        <span className="text-[10px] text-violet-400">
                          Card {detail.performance.card_sales}
                        </span>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                  {[
                    ...
                    {
                      label: "Total Units",
                      value: detail.inventory.total_units?.toLocaleString(),
                      color: "",
                    },
                    {
                      label: "Stock Value",
                      value: ghc(detail.inventory.inventory_value),
                      color: "text-primary",
                    },
                    {
                      label: "Low Stock",
                      value: detail.inventory.low_stock,
                      color: "text-amber-400",
                    },
                    {
                      label: "Out of Stock",
                      value: detail.inventory.out_of_stock,
                      color: "text-red-400",
                    },
                  ].map((k) => (
                    <Card key={k.label}>
                      ...
                      <TableBody>
                        ...
                        {detail.topProducts.map((p, i) => (
                          <TableRow key={p.id} className="text-xs">
                            ...
                            <TableCell className="text-right pr-3 font-mono">
                              {ghc(p.revenue)}
                            </TableCell>
                          </TableRow>
                        ))}
