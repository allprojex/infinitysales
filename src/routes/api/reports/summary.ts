  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const [{ data: salesAll, error: e1 }, { count: custCount }, { count: prodCount }] =
          await Promise.all([
            salesQ,
            custQ,
        ...
        const prevMonth = completed.filter(
          (r) => r.sold_at >= prevMonthStart && r.sold_at < monthStart,
        );
        ...
        return json({
          ...
          revenueGrowth:
            revenuePrev > 0 ? ((revenueThisMonth - revenuePrev) / revenuePrev) * 100 : 0,
