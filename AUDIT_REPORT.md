...
- **ISSUE-015 (High, fully resolved — database-level fix applied, then live-verified end-to-end via Playwright against production):** the `profiles` table's `prevent_sensitive_profile_changes()` trigger blocked every admin's Active/Disabled toggle (and must-change-password/2FA/email changes) because `auth.uid()` is always `NULL` for the server's service-role client, so `has_role(auth.uid(),'admin')` always evaluated false. Fixed via migration `20260722120000_allow_service_role_profile_field_updates.sql` (also recorded remotely as version `20260722130734` — filename/version mismatch flagged for reconciliation), adding an `auth.role() = 'service_role'` allowance. Verified directly against production with transaction-scoped, rolled-back SQL: service-role update now succeeds, a non-admin's own direct update is still correctly blocked, the pre-existing authenticated-admin path is unaffected, and all RLS policies/triggers on `profiles`/`user_roles` are unchanged. Subsequently live-verified end-to-end (Playwright, logged in as `infinitytechub@outlook.com` on production): toggling a real user's Active/Blocked status through the actual admin panel now returns `200` (previously the exact reported `500`), with the account reverted and confirmed via SQL to match its pre-test state afterward.
