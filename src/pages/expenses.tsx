...
import { Receipt, Plus, Pencil, Trash2, RefreshCw, Search, TrendingDown, Calendar, ChevronLeft, ChevronRight } from "lucide-react";
...
const CAT_COLORS: Record<string, string> = {
  rent_lease: "bg-purple-500/15 text-purple-300", utilities: "bg-blue-500/15 text-blue-300",
  staff_transport: "bg-sky-500/15 text-sky-300", marketing: "bg-pink-500/15 text-pink-300",
  office_supplies: "bg-orange-500/15 text-orange-300", maintenance: "bg-yellow-500/15 text-yellow-300",
  security: "bg-red-500/15 text-red-300", insurance: "bg-teal-500/15 text-teal-300",
  communication: "bg-cyan-500/15 text-cyan-300", salaries: "bg-green-500/15 text-green-300",
  taxes: "bg-rose-500/15 text-rose-300", miscellaneous: "bg-slate-500/15 text-slate-300",
};
...
/* ── Types ──────────────────────────────────────────────── */
interface Expense {
  id: number; title: string; category: string; amount: string;
  expenseDate: string; description: string | null; receiptNote: string | null;
  status: string; createdBy: string | null; createdAt: string;
}
interface ExpenseResp { data: Expense[]; total: number; page: number; limit: number; }
interface StatResp {
...
const EMPTY_FORM = {
  ...
  description: "", receiptNote: "", status: "pending",
};
...
export default function Expenses() {
  ...
  const openEdit = (e: Expense) => {
    ...
    setForm({
      ...
      receiptNote: e.receiptNote ?? "", status: e.status,
    });
  };

  const buildBody = () => ({
    title: form.title, category: form.category,
    amount: parseFloat(form.amount), expenseDate: form.expenseDate,
    description: form.description || null, receiptNote: form.receiptNote || null,
    status: form.status,
  });
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input className="h-8 text-xs" type="date" value={form.expenseDate}
            onChange={e => setForm(f => ({ ...f, expenseDate: e.target.value }))} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-muted-foreground font-medium mb-1 block">Amount (GHS) *</label>
          <Input className="h-8 text-xs" type="number" min="0.01" step="0.01" value={form.amount}
      ...
      <div>
        <label className="text-xs text-muted-foreground font-medium mb-1 block">Receipt / Reference Note</label>
        <Input className="h-8 text-xs" value={form.receiptNote}
    ...
    <div className="p-4 sm:p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Track operating costs — rent, utilities, marketing and more</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="border-red-500/20 bg-red-500/5 col-span-2 sm:col-span-1">
          <CardContent className="pt-4 pb-3">
            ...
            <p className="text-[10px] text-muted-foreground">{stats?.thisMonth?.count ?? 0} expenses</p>
          </CardContent>
        </Card>
        <Card className="border-orange-500/20 bg-orange-500/5 col-span-2 sm:col-span-1">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
              <TrendingDown className="h-3.5 w-3.5" />This Year
            </div>
            <p className="text-xl font-bold text-orange-400">{GHS(stats?.thisYear?.total ?? 0)}</p>
            <p className="text-[10px] text-muted-foreground">{stats?.thisYear?.count ?? 0} expenses</p>
          </CardContent>
        </Card>
      ...
      {topCats.length > 0 && (
        <Card>
          <CardContent className="pt-4 pb-4">
            <p className="text-xs font-medium text-muted-foreground mb-3">Top Spending Categories (This Year)</p>
            <div className="space-y-2">
              {topCats.map(c => (
                <div key={c.category} className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-right w-24 shrink-0 font-medium">{GHS(c.total)}</span>
                  <span className="text-[10px] text-muted-foreground w-12 shrink-0">{c.count} txn{c.count !== 1 ? "s" : ""}</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative flex-1 min-w-40">
          ...
          <Input className="h-8 text-xs pl-8" placeholder="Search expenses…"
        ...
        <Input className="h-8 text-xs w-32" type="date" value={startDate}
          onChange={e => { setStartDate(e.target.value); setPage(1); }} title="From date" />
        <Input className="h-8 text-xs w-32" type="date" value={endDate}
          onChange={e => { setEndDate(e.target.value); setPage(1); }} title="To date" />
            ...
            {rows.map(e => (
              <TableRow key={e.id}>
                <TableCell>
                  ...
                  {e.description && (
                    <div className="text-[10px] text-muted-foreground truncate max-w-48">{e.description}</div>
                  )}
                </TableCell>
                <TableCell>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${CAT_COLORS[e.category] ?? ""}`}>
                    {catLabel(e.category)}
                  </span>
                </TableCell>
                <TableCell className="text-sm">
                  {new Date(e.expenseDate).toLocaleDateString("en-GH", { day: "2-digit", month: "short", year: "numeric" })}
                </TableCell>
                <TableCell className="text-right font-semibold text-sm text-red-400">
                  {GHS(Number(e.amount))}
                </TableCell>
                <TableCell>
                  <Badge variant="outline" className={`text-[10px] capitalize ${STATUS_COLORS[e.status] ?? ""}`}>
                    {e.status}
                  </Badge>
                </TableCell>
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={createExp.isPending || !form.title.trim() || !form.amount || !form.expenseDate}>
      ...
      <Dialog open={!!editExp} onOpenChange={open => !open && setEditExp(null)}>
        <DialogContent className="max-w-md">
          ...
          {FormBody()}
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditExp(null)}>Cancel</Button>
            <Button
