  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        let q = sb.from("contacts").select("*", { count: "exact" }).eq("user_id", user.id).order("id", { ascending: false }).range(offset, offset + limit - 1);
      ...
      POST: async ({ request }) => {
        ...
        const { data, error } = await sb.from("contacts").insert({ ...apiToRow(body) as any, user_id: user.id }).select("*").single();
        if (error) return errorJson(500, error.message);
