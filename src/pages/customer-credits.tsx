...
export default function CustomerCredits() {
  ...
  const { data: accounts = [], isLoading } = useQuery<CreditAccount[]>({
    ...
    queryFn: () => {
      ...
      return customFetch<any>(`/api/customer-credits?${params}`).then((d) =>
  ...
  const setupMutation = useMutation({
    mutationFn: (cid: number) =>
      customFetch(`/api/customer-credits/customer/${cid}/setup`, {
        ...
        body: JSON.stringify({
          creditLimit: Number(setupForm.creditLimit),
    ...
    <div className="flex h-full overflow-hidden">
          ...
          <div className="px-3 py-2 grid grid-cols-2 gap-1.5 border-b">
            ...
            <div className="rounded bg-muted/40 px-2 py-1">
              ...
              <p className="text-xs font-bold">
                {summary.accounts_with_balance} / {summary.total_accounts}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="px-2 pt-2 space-y-2">
          ...
          >
            <TabsList className="w-full h-7">
              ...
              <TabsTrigger value="suspended" className="flex-1 text-xs h-6">
                Suspended
              </TabsTrigger>
            </TabsList>
          </Tabs>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground truncate">
                    {a.customer_company ?? a.customer_email}
                  </p>
                  <div className="flex items-center justify-between mt-0.5">
                    <p
                      className={`text-[11px] font-mono font-semibold ${owed > 0 ? "text-red-400" : "text-muted-foreground"}`}
                    >
                      {ghc(owed)} owed
                    </p>
                  ...
                  {pct !== null && (
                    <div className="mt-1 h-1 rounded bg-muted overflow-hidden">
                      <div
                        className={`h-full rounded transition-all ${pct > 80 ? "bg-red-500" : pct > 50 ? "bg-amber-500" : "bg-emerald-500"}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  )}
        ...
        {!selected ? (
          <div className="flex-1 flex items-center justify-center flex-col gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              Select a credit account or enable one for a customer
            </p>
            <Button
              size="sm"
              onClick={() => {
          ...
          <>
            ...
            <div className="border-b px-5 py-3 flex items-center justify-between shrink-0">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <Button
                  ...
                  onClick={() => {
                    setSetupForm({
                      creditLimit: detail.credit.credit_limit,
                      status: detail.credit.status,
                      notes: detail.credit.notes ?? "",
                    });
                    setSetupDlg(true);
                  }}
                >
            ...
            <div className="px-5 py-3 grid grid-cols-2 sm:grid-cols-4 gap-3 border-b shrink-0">
              <Card
                className={`border-${outstanding > 0 ? "red" : "border"}-500/20 bg-${outstanding > 0 ? "red" : "muted"}/5`}
              >
                <CardContent className="pt-3 pb-2">
                  <p className="text-[10px] text-muted-foreground">Outstanding</p>
                  <p
                    className={`text-xl font-bold ${outstanding > 0 ? "text-red-400" : "text-muted-foreground"}`}
                  >
                    {ghc(outstanding)}
                  </p>
                  {utilPct !== null && (
                    <p className="text-[10px] text-muted-foreground">
                      {utilPct.toFixed(0)}% of limit used
                    </p>
                  )}
                </CardContent>
              </Card>
              <Card className="border-border">
                <CardContent className="pt-3 pb-2">
                  ...
                  {available !== null && (
                    <p className="text-[10px] text-emerald-400">{ghc(available)} available</p>
                  )}
                </CardContent>
              </Card>
            ...
            {utilPct !== null && (
              <div className="px-5 py-2 shrink-0 border-b">
                ...
                <div className="h-2 rounded bg-muted overflow-hidden">
                  <div
                    className={`h-full rounded transition-all ${utilPct > 80 ? "bg-red-500" : utilPct > 50 ? "bg-amber-500" : "bg-emerald-500"}`}
                    style={{ width: `${utilPct}%` }}
                  />
                </div>
                {utilPct > 80 && (
                  <p className="text-[10px] text-red-400 mt-1 flex items-center gap-1">
                    ...
                    High utilisation — consider reviewing credit limit
                  </p>
                )}
              </div>
            )}
                ...
                <TableBody>
                  ...
                  {detail.transactions.map((t) => (
                    <TableRow key={t.id} className="text-xs">
                      ...
                      <TableCell
                        className={`text-right font-mono font-semibold ${txColor(t.type)}`}
                      >
                        {txSign(t.type)}
                        {ghc(Math.abs(Number(t.amount)))}
                      </TableCell>
      ...
      <Dialog open={setupOpen} onOpenChange={setSetupOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Enable Credit Account</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-1">
            <div className="space-y-1.5">
              ...
              />
              <p className="text-[10px] text-muted-foreground">
                You can find the customer ID on the Customers page.
              </p>
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                value={setupForm.creditLimit}
                onChange={(e) => setSetupForm((f) => ({ ...f, creditLimit: e.target.value }))}
              />
      ...
      <Dialog open={setupDlg} onOpenChange={setSetupDlg}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Credit Account Settings</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-1">
            <div className="space-y-1.5">
              <Label className="text-xs">Credit Limit (₵)</Label>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                {["active", "suspended", "closed"].map((s) => (
                  <button
                    key={s}
                    onClick={() => setSetupForm((f) => ({ ...f, status: s }))}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSetupDlg(false)}>
              Cancel
            </Button>
            <Button
              onClick={() => setupMutation.mutate(selected!)}
      ...
      <Dialog open={payDlg} onOpenChange={setPayDlg}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Receipt #, cheque #, etc."
              />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Notes</Label>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPayDlg(false)}>
              Cancel
            </Button>
            <Button
      ...
      <Dialog open={chargeDlg} onOpenChange={setChargeDlg}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Charge to Account</DialogTitle>
          </DialogHeader>
          {detail && creditLimit > 0 && (
            <div className="rounded-lg bg-muted/40 px-3 py-2 text-xs mb-2 flex justify-between">
              <span className="text-muted-foreground">Available credit</span>
              <span className="font-semibold text-emerald-400">
                {available !== null ? ghc(available) : "—"}
              </span>
            </div>
          )}
          <div className="space-y-4 py-1">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setChargeDlg(false)}>
              Cancel
            </Button>
            <Button
              onClick={() => chargeMutation.mutate()}
              disabled={
                !chargeForm.amount || Number(chargeForm.amount) <= 0 || chargeMutation.isPending
              }
            >
      ...
      <Dialog open={adjDlg} onOpenChange={setAdjDlg}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-xs text-muted-foreground -mt-1 mb-2">
            Enter a positive number to increase the balance, negative to reduce it.
          </p>
          <div className="space-y-4 py-1">
            <div className="space-y-1.5">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAdjDlg(false)}>
              Cancel
            </Button>
            <Button
