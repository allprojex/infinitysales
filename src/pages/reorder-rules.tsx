...
interface ReorderRule {
  ...
  category: string | null; current_stock: number; unit_price: string;
  reorder_point: number; reorder_qty: number;
  preferred_supplier_id: number | null; preferred_supplier_name: string | null;
  is_active: boolean; auto_create_po: boolean; last_triggered: string | null;
...
const emptyForm = { productId: "", reorderPoint: "10", reorderQty: "50", preferredSupplierId: "", isActive: true, autoCreatePo: false };
...
export default function ReorderRules() {
  ...
  const generateMutation = useMutation({
    mutationFn: () => customFetch("/api/reorder-rules/generate-po", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({}) }),
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Automatic reorder triggers and purchase order generation</p>
        </div>
        <div className="flex gap-2">
          {needsReorder.length > 0 && (
            <Button variant="outline" className="gap-1.5 text-amber-400 border-amber-500/30 hover:text-amber-300" onClick={() => setGenConfirm(true)}>
              <Zap className="h-4 w-4" />Generate POs ({needsReorder.length})
            </Button>
          )}
          <Button className="gap-1.5" onClick={() => { setEditTarget(null); setForm(emptyForm); setDlgOpen(true); }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <Card className={needsReorder.length > 0 ? "border-amber-500/20 bg-amber-500/5" : "border-border"}>
          <CardContent className="pt-3 pb-2">
            <p className="text-[10px] text-muted-foreground">Need Reordering</p>
            <p className={`text-xl font-bold ${needsReorder.length > 0 ? "text-amber-400" : "text-muted-foreground"}`}>{needsReorder.length}</p>
      ...
      <div className="rounded-lg border overflow-hidden">
        <Table>
          ...
          <TableBody>
            ...
            {rules.map(r => (
              <TableRow key={r.id} className={`text-xs group ${r.needs_reorder && r.is_active ? "bg-amber-500/5" : ""}`}>
                <TableCell className="pl-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      {r.sku && <p className="text-[10px] text-muted-foreground font-mono">{r.sku}</p>}
                    </div>
                  </div>
                </TableCell>
                <TableCell className="text-muted-foreground">{r.category ?? "—"}</TableCell>
                <TableCell className={`text-right font-mono font-semibold ${r.needs_reorder ? "text-amber-400" : "text-foreground"}`}>
                  {r.current_stock}
                </TableCell>
                ...
                <TableCell className="text-muted-foreground">{r.preferred_supplier_name ?? "Any"}</TableCell>
                <TableCell>
                  <Badge variant="outline" className={`text-[10px] py-0 ${r.auto_create_po ? "text-primary border-primary/30" : "text-muted-foreground"}`}>
                    {r.auto_create_po ? "Yes" : "No"}
                  </Badge>
                </TableCell>
                <TableCell>
                  <Badge variant="outline" className={`text-[10px] py-0 ${r.is_active ? "text-emerald-400 border-emerald-500/30" : "text-muted-foreground"}`}>
                ...
                <TableCell className="pr-4 text-right">
                  <div className="flex gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button size="icon" variant="ghost" className="h-6 w-6"
                      onClick={() => { setEditTarget(r); setForm({ productId: String(r.product_id), reorderPoint: String(r.reorder_point), reorderQty: String(r.reorder_qty), preferredSupplierId: r.preferred_supplier_id ? String(r.preferred_supplier_id) : "", isActive: r.is_active, autoCreatePo: r.auto_create_po }); setDlgOpen(true); }}>
      ...
      <Dialog open={dlgOpen} onOpenChange={setDlgOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>{editTarget ? "Edit Reorder Rule" : "Add Reorder Rule"}</DialogTitle></DialogHeader>
          <div className="space-y-3 py-1">
            ...
            {editTarget && (
              <div className="rounded-lg bg-muted/40 p-2 text-xs text-muted-foreground">Product: <span className="font-semibold text-foreground">{editTarget.product_name}</span></div>
            )}
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs">Reorder At (units)</Label>
                <Input type="number" min="0" value={form.reorderPoint} onChange={e => setForm(f => ({ ...f, reorderPoint: e.target.value }))} />
                <p className="text-[10px] text-muted-foreground">When stock falls to this level</p>
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Order Quantity</Label>
                <Input type="number" min="1" value={form.reorderQty} onChange={e => setForm(f => ({ ...f, reorderQty: e.target.value }))} />
                <p className="text-[10px] text-muted-foreground">Units to include in PO</p>
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Preferred Supplier</Label>
              <select value={form.preferredSupplierId} onChange={e => setForm(f => ({ ...f, preferredSupplierId: e.target.value }))}
            ...
            <div className="flex gap-6 pt-1">
              ...
              <div className="flex items-center gap-2">
                <Switch id="autoPo" checked={form.autoCreatePo} onCheckedChange={v => setForm(f => ({ ...f, autoCreatePo: v }))} />
                <Label htmlFor="autoPo" className="text-xs cursor-pointer">Auto-generate PO</Label>
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setDlgOpen(false); setEditTarget(null); }}>Cancel</Button>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will create draft purchase orders for <strong>{needsReorder.length}</strong> product(s) that are below their reorder point. Orders will be grouped by supplier.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => generateMutation.mutate()} disabled={generateMutation.isPending}>
      ...
      <AlertDialog open={!!delTarget} onOpenChange={open => !open && setDelTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Delete rule for <strong>{delTarget?.product_name}</strong>?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
