...
export default function Departments() {
  ...
  const createDept = useMutation({
    mutationFn: (body: object) =>
      customFetch("/api/departments", { method: "POST", body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const updateDept = useMutation({
    mutationFn: ({ id, body }: { id: number; body: object }) =>
      customFetch(`/api/departments/${id}`, { method: "PUT", body: JSON.stringify(body) }),
    onSuccess: () => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Manage organisational departments and budgets
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardContent className="pt-10 pb-10 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Add your first department to organise your workforce.
            </p>
            {isAdmin && (
              <Button className="mt-4 gap-1.5" size="sm" onClick={() => setShowForm(true)}>
                <Plus className="h-3.5 w-3.5" />
                New Department
              </Button>
            )}
            ...
            <CardContent className="pt-0 space-y-1.5">
              ...
              {d.budget && (
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  <span className="font-medium text-emerald-400">{GHS(Number(d.budget))}</span>
                </div>
              )}
            </CardContent>
          </Card>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => createDept.mutate(buildBody())}
              disabled={createDept.isPending || !form.name.trim()}
            >
      ...
      <Dialog open={!!editDept} onOpenChange={(open) => !open && setEditDept(null)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => editDept && updateDept.mutate({ id: editDept.id, body: buildBody() })}
              disabled={updateDept.isPending || !form.name.trim()}
            >
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This cannot be undone. Employees in this department will not be deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => deleteId !== null && deleteDept.mutate(deleteId)}
