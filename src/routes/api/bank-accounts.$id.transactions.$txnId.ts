  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const { data, error } = await sb.from("bank_transactions").select("*")
          .eq("user_id", auth.user.id).eq("bank_account_id", params.id).eq("id", params.txnId).maybeSingle();
      ...
      PATCH: async ({ request, params }) => {
        ...
        const { data, error } = await sb.from("bank_transactions").update(apiToRow(body) as any)
          .eq("user_id", auth.user.id).eq("id", params.txnId).select("*").single();
        if (error) return json({ message: error.message }, { status: 500 });
        return json(rowToApi(data));
      },
      DELETE: async ({ request, params }) => {
        ...
        const { error } = await sb.from("bank_transactions").delete()
          .eq("user_id", auth.user.id).eq("id", params.txnId);
        if (error) return json({ message: error.message }, { status: 500 });
        return json({ ok: true });
      },
    },
