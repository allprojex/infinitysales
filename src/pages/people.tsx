...
function useContacts(search: string) {
  return useQuery<{ data: Contact[]; total: number }>({
    ...
    queryFn: () =>
      customFetch(`/api/contacts?limit=50${search ? `&search=${encodeURIComponent(search)}` : ""}`),
  });
}
    ...
    <div className="space-y-4 pt-1 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div>
        ...
        <Input
          id="person-address"
          name="address"
          placeholder="12 Airport Road, Accra"
          value={address}
                  ...
                  <TableRow key={c.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <TableCell>
                      <div className="text-sm">
                        ...
                        {c.role && (
                          <div className="text-muted-foreground text-xs mt-0.5">{c.role}</div>
                        )}
                        {!c.company && !c.role && (
                      ...
                      <div className="text-sm space-y-0.5">
                        {c.email && (
                          <div className="flex items-center">
                            <Mail className="h-3 w-3 mr-1.5 text-muted-foreground shrink-0" />
                            {c.email}
                          </div>
                        )}
                        {parsePhones(c.phone)
