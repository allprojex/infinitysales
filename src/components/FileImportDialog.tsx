...
const COLUMN_GUIDE: Record<ImportType, { col: string; description: string; required: boolean }[]> =
  {
    sales: [
      ...
      {
        col: "customer_name",
        description: "Required if customer_email is missing",
        required: false,
      },
      { col: "customer_email", description: "Must be a valid email address", required: false },
      {
        col: "product_name",
        description: "Looked up in catalogue; missing items keep the typed name",
        required: true,
      },
      { col: "quantity", description: "Whole number greater than 0", required: true },
      {
        col: "unit_price",
        description: "Optional — falls back to catalogue price",
        required: false,
      },
    ...
    purchases: [
      {
        col: "order_ref",
        description: "Groups multiple rows into one purchase order",
        required: true,
      },
      {
        col: "supplier",
        description: "Created automatically if it doesn't exist yet",
        required: false,
      },
      ...
      {
        col: "status",
        description: "draft · ordered · pending · received · cancelled",
        required: false,
      },
      { col: "notes", description: "Free-form notes", required: false },
    ],
...
export function FileImportDialog({ type, open, onClose, onSuccess }: Props) {
  ...
  const commit = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error((data as any)?.error ?? (data as any)?.message ?? "Import failed");
      setResult(data);
      setStep("done");
      if (data.imported > 0) {
        onSuccess();
        ...
        toast({
          ...
          description: data.errors[0] ?? data.message,
        });
      }
    } catch (e: any) {
      toast({
    ...
    >
      <DialogContent className="sm:max-w-[760px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "pick" &&
              `Upload CSV or Excel (.xlsx) files. Each file is parsed and validated before any data is saved.`}
            {step === "preview" &&
              `Review how each row will map to the database. Rows with errors are skipped automatically — fix and re-upload to include them.`}
        ...
        {step === "pick" && (
          <div className="space-y-4">
            <div className="flex items-start gap-2 p-3 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 text-xs text-blue-700 dark:text-blue-300">
              ...
              <span>
                Files are validated row-by-row. You'll see a preview of every mapped value before
                anything is saved.
              </span>
            </div>
            ...
            {files.length > 0 && (
              <div className="space-y-1.5">
                {files.map((f) => (
                  <div
                    key={f.name}
                    className="flex items-center gap-2.5 px-3 py-2 rounded-lg border bg-muted/20"
                  >
                ...
                <div className="border-t px-4 pb-4 pt-3 space-y-1.5">
                  {COLUMN_GUIDE[type].map((g) => (
                    <div key={g.col} className="flex items-start gap-2 text-xs">
                      <code className="font-mono bg-muted px-1.5 py-0.5 rounded text-[11px] flex-shrink-0">
                        {g.col}
                      </code>
                      {g.required && (
                        <span className="text-[10px] font-semibold text-red-600 mt-0.5 flex-shrink-0">
                          REQ
                        </span>
                      )}
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {busy
                    ? "Validating…"
                    : `Preview ${files.length || ""} file${files.length !== 1 ? "s" : ""}`.trim()}
                </Button>
              </div>
            </div>
          </div>
        ...
        {step === "preview" && preview && (
          <div className="space-y-4">
            <div className="grid grid-cols-4 gap-2">
              ...
              <SummaryStat
                label="Errors"
                value={preview.totals.errors}
                tone={preview.totals.errors ? "error" : "neutral"}
              />
              <SummaryStat
                label="Warnings"
                value={preview.totals.warnings}
                tone={preview.totals.warnings ? "warning" : "neutral"}
              />
            </div>
                ...
                return (
                  <div key={f.file} className="rounded-xl border overflow-hidden">
                    ...
                    >
                      ...
                      {f.warningCount > 0 && (
                        <span className="text-[11px] flex items-center gap-1 text-amber-600">
                      ...
                      <div className="border-t bg-muted/10">
                        {f.unmappedHeaders.length > 0 && (
                          <div className="px-3 py-2 text-[11px] text-amber-700 dark:text-amber-400 bg-amber-50/50 dark:bg-amber-950/20 border-b border-amber-200/40">
                            <strong>Unrecognised columns:</strong> {f.unmappedHeaders.join(", ")} —
                        ...
                        {f.fileWarnings.map((w, i) => (
                          <div
                            key={i}
                            className="px-3 py-2 text-[11px] text-amber-700 dark:text-amber-400 border-b border-amber-200/40"
                          >
                            {w}
                          </div>
                        ))}
                          ...
                          <div className="max-h-[300px] overflow-y-auto divide-y">
                            ...
                            {visibleRows.length > 100 && (
                              <div className="px-3 py-2 text-[11px] text-center text-muted-foreground italic">
                                …and {visibleRows.length - 100} more row(s). Commit to import all
                                valid rows.
                              </div>
                            )}
                          </div>
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  className="rounded-full gap-2"
                  disabled={busy || preview.totals.valid === 0}
                  onClick={commit}
                >
                  {busy ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ...
                  {busy
                    ? "Importing…"
                    : `Commit ${preview.totals.valid} valid row${preview.totals.valid !== 1 ? "s" : ""}`}
                </Button>
              </div>
            </div>
          </div>
                ...
                {result.blocked.length > 0 && (
                  <div className="flex items-center gap-1.5 text-red-600 dark:text-red-400">
                    ...
                    <span className="text-sm font-semibold">
                      {result.blocked.length} file(s) blocked
                    </span>
                  </div>
                )}
              </div>
              {(result.errors.length > 0 || result.blocked.length > 0) && (
                <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
                  {[
                    ...
                    ...result.errors.map((m) => ({ kind: "error" as const, text: m })),
                  ].map((m, i) => (
                    <div
                      key={i}
                      className={cn(
...
function SummaryStat({
  ...
  const toneClass =
    tone === "success"
      ...
      : tone === "error"
        ? "border-red-200 bg-red-50/50 dark:bg-red-950/20 text-red-700 dark:text-red-300"
        : tone === "warning"
...
function RowCard({ row }: { row: PreviewRow }) {
  ...
  const mappedEntries = row.mapped
    ? Object.entries(row.mapped).filter(([, v]) => v !== null && v !== "" && v !== undefined)
    : [];
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1.5">
        ...
        {isError ? (
          ...
          <span className="flex items-center gap-1 text-emerald-600 font-medium">
            <CheckCircle2 className="h-3 w-3" /> will import
          </span>
        )}
        {isWarn && (
      ...
      {row.errors.map((e, i) => (
        <div key={`e-${i}`} className="text-red-700 dark:text-red-400 leading-relaxed">
          • {e}
        </div>
      ))}
      {row.warnings.map((w, i) => (
        <div key={`w-${i}`} className="text-amber-700 dark:text-amber-400 leading-relaxed">
          • {w}
        </div>
      ))}
    </div>
