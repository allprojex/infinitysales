  ...
  server: {
    handlers: {
      ...
      PUT: async ({ request, params }) => {
        ...
        const { data, error } = await sb.from("contacts").update(apiToRow(body) as any).eq("user_id", user.id).eq("id", Number(params.id)).select("*").single();
        if (error) return errorJson(500, error.message);
        return json(rowToApi(data));
      },
      DELETE: async ({ request, params }) => {
        const { user, response } = await requireUser(request);
        if (!user) return response;
        const { error } = await sb.from("contacts").delete().eq("user_id", user.id).eq("id", Number(params.id));
        if (error) return errorJson(500, error.message);
        return json({ ok: true });
      },
    },
