...
export default function VATReport() {
  ...
  const { data, isLoading } = useQuery<VATReport>({
    ...
    queryFn: () =>
      customFetch(`/api/vat-report?startDate=${applied.startDate}&endDate=${applied.endDate}`),
    staleTime: 30_000,
  ...
  const saveRates = useMutation({
    mutationFn: () =>
      customFetch("/api/vat-report/rates", {
        method: "PUT",
    ...
    <div className="p-5 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Tax breakdown on completed sales (VAT, NHIL, GETFund levies)
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-lg border bg-muted/20 p-3">
        ...
        {data && (
          <div className="ml-auto text-[11px] text-muted-foreground">
            Rates: VAT {pct(data.rates.vatRate)} · NHIL {pct(data.rates.nhilRate)} · GETFund{" "}
            {pct(data.rates.getfundRate)}
            ...
            <Card className="border-primary/20 bg-primary/5">
              <CardContent className="pt-3 pb-2">
                ...
                <p className="text-[10px] text-muted-foreground">
                  {data.summary.totalSales} completed sales
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="border-amber-500/20 bg-amber-500/5">
              <CardContent className="pt-3 pb-2">
                ...
                <p className="text-xl font-bold text-amber-400">
                  {ghc(data.summary.totalTaxCollected)}
                </p>
                <p className="text-[10px] text-muted-foreground">
                  {pct(data.rates.totalTaxRate)} effective rate
                </p>
              </CardContent>
            </Card>
            <Card>
                ...
                <div className="space-y-0.5 mt-1">
                  ...
                  <div className="flex justify-between text-[11px]">
                    <span className="text-muted-foreground">
                      GETFund ({pct(data.rates.getfundRate)})
                    </span>
                  ...
                  {data.rates.covidLevy > 0 && (
                    <div className="flex justify-between text-[11px]">
                      <span className="text-muted-foreground">
                        COVID Levy ({pct(data.rates.covidLevy)})
                      </span>
                  ...
                  <TableBody>
                    ...
                    {data.monthly.map((m) => (
                      <TableRow key={m.month_key} className="text-xs">
                        ...
                        <TableCell className="text-right font-mono text-primary pr-4">
                          {ghc(Number(m.nhil) + Number(m.getfund))}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                  ...
                  <TableBody>
                    ...
                    {data.byMethod.map((m) => (
                      <TableRow key={m.method} className="text-xs">
                        <TableCell className="pl-4 capitalize font-medium">
                          {m.method.replace("_", " ")}
                        </TableCell>
                        <TableCell className="text-right text-muted-foreground">
                          {m.sales_count}
                        ...
                        <TableCell className="text-right font-mono">{ghc(m.gross)}</TableCell>
                        <TableCell className="text-right font-mono text-amber-400 pr-4">
                          {ghc(m.vat)}
                        </TableCell>
                      </TableRow>
                    ))}
      ...
      <Dialog open={ratesDlg} onOpenChange={setRatesDlg}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Configure Ghana Tax Rates</DialogTitle>
          </DialogHeader>
          <p className="text-xs text-muted-foreground">
            These rates are used to compute the VAT/NHIL/GETFund breakdown from inclusive sale
            totals.
          </p>
          <div className="space-y-3 py-1">
            {[
              { key: "vatRate", label: "VAT Rate (%)", desc: "Standard VAT (currently 15%)" },
              {
                key: "nhilRate",
                label: "NHIL Rate (%)",
                desc: "National Health Insurance Levy (2.5%)",
              },
              {
                key: "getfundRate",
                label: "GETFund Rate (%)",
                desc: "Ghana Education Trust Fund (2.5%)",
              },
            ...
            <div className="rounded-lg bg-muted/40 p-2 text-[11px]">
              ...
              <span className="font-bold text-primary">
                {(
                  Number(ratesForm.vatRate) +
                  Number(ratesForm.nhilRate) +
                  Number(ratesForm.getfundRate) +
                  Number(ratesForm.covidLevy)
                ).toFixed(1)}
                %
              </span>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRatesDlg(false)}>
              Cancel
            </Button>
