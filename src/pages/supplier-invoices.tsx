...
export default function SupplierInvoices() {
  ...
  const hasInvalidDateRange = Boolean(
    form.issueDate && form.dueDate && form.dueDate < form.issueDate,
  );
  ...
  const { data: invoicesResp, isLoading } = useQuery<Invoice[] | { data: Invoice[] }>({
    ...
    queryFn: () =>
      customFetch(`/api/supplier-invoices${filterTab !== "all" ? `?status=${filterTab}` : ""}`),
    refetchInterval: 60_000,
  });
  const invoices: Invoice[] = Array.isArray(invoicesResp)
    ? invoicesResp
    : (invoicesResp?.data ?? []);
  ...
  const { data: suppliersResp } = useQuery<{ data: Array<{ id: number; name: string }> }>({
    ...
    queryFn: () =>
      customFetch(
        `/api/suppliers?limit=50${form.supplierName ? `&search=${encodeURIComponent(form.supplierName)}` : ""}`,
      ),
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return editTarget
        ? customFetch(`/api/supplier-invoices/${editTarget.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          })
        : customFetch("/api/supplier-invoices", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          });
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Track and manage incoming supplier invoices and payments
          </p>
        </div>
        <Button
          className="gap-1.5"
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card className="border-amber-500/20 bg-amber-500/5">
            <CardContent className="pt-3 pb-2">
              ...
              <p className="text-[10px] text-muted-foreground">
                {summary.unpaid + summary.partial} invoices unpaid
              </p>
            </CardContent>
          </Card>
          ...
          <Card
            className={summary.overdue > 0 ? "border-red-500/20 bg-red-500/5" : "border-border"}
          >
            <CardContent className="pt-3 pb-2">
              <p className="text-[10px] text-muted-foreground">Overdue</p>
              <p
                className={`text-xl font-bold ${summary.overdue > 0 ? "text-red-400" : "text-muted-foreground"}`}
              >
                {summary.overdue}
              </p>
      ...
      <Tabs value={filterTab} onValueChange={setFilterTab}>
        <TabsList className="h-8">
          {["all", "unpaid", "partial", "overdue", "paid", "disputed"].map((s) => (
            <TabsTrigger key={s} value={s} className="text-xs h-7 capitalize">
              {s}
            </TabsTrigger>
          ))}
        </TabsList>
            ...
            {invoices.map((inv) => {
              ...
              return (
                <TableRow key={inv.id} className="text-xs group">
                  ...
                  <TableCell
                    className={`text-right font-mono font-semibold ${balance > 0 ? "text-amber-400" : "text-emerald-400"}`}
                  >
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`text-[10px] py-0 capitalize ${STATUS_COLORS[inv.status] ?? ""}`}
                    >
                      {inv.status}
                    </Badge>
                  </TableCell>
                      ...
                      {inv.status !== "paid" && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setPayForm({
                              ...emptyPayForm,
                              amountPaid: String(Number(inv.total) - Number(inv.amount_paid)),
                            });
                          }}
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editTarget ? "Edit Invoice" : "Add Supplier Invoice"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-1 max-h-[70vh] overflow-y-auto pr-1">
                      ...
                      [...suppliersResp.data]
                        ...
                        .map((s) => (
                          <button
                            ...
                            onClick={() => {
                              setForm((f) => ({ ...f, supplierName: s.name }));
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...
                      dueDate: f.dueDate && f.dueDate < e.target.value ? "" : f.dueDate,
                    }))
                  }
                />
              </div>
            ...
            {hasInvalidDateRange && (
              <p className="text-xs text-destructive">Due date cannot be before the issue date.</p>
            )}
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setForm((f) => ({ ...f, subtotal: st, total: String(Number(st) + tax) }));
                  }}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setForm((f) => ({
                      ...f,
                      taxAmount: tax,
                      total: String(Number(f.subtotal) + Number(tax)),
                    }));
                  }}
                />
              </div>
        ...
        <DialogContent className="max-w-sm">
            ...
            <div className="space-y-3 py-1">
              <div className="rounded-lg bg-muted/40 p-3 text-xs space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-semibold">{payTarget.invoice_number}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Supplier</span>
                  <span>{payTarget.supplier_name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Total</span>
                  <span className="font-mono">{ghc(payTarget.total)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Balance due</span>
                  <span className="font-mono font-bold text-amber-400">
                    {ghc(Number(payTarget.total) - Number(payTarget.amount_paid))}
                  </span>
                </div>
              </div>
              <div className="space-y-1.5">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !payForm.amountPaid || Number(payForm.amountPaid) <= 0 || payMutation.isPending
              }
            >
      ...
      <AlertDialog open={!!delTarget} onOpenChange={(open) => !open && setDelTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete invoice <strong>{delTarget?.invoice_number}</strong>? This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
