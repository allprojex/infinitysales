  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const { data, error } = await sb.from("tasks").select("*").eq("user_id", user.id).eq("project_id", params.id).order("created_at", { ascending: false });
      ...
      POST: async ({ request, params }) => {
        ...
        const { data, error } = await sb.from("tasks").insert(row as any).select("*").single();
        if (error) return errorJson(500, error.message);
