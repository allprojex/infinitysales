...
export default function Promotions() {
  ...
  const createPromo = useMutation({
    mutationFn: (body: object) =>
      customFetch("/api/promotions", { method: "POST", body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const patchStatus = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      customFetch(`/api/promotions/${id}/status`, {
        method: "PATCH",
  ...
  const UsageBar = ({ p }: { p: Promotion }) => {
    if (!p.usageLimit)
      return <span className="text-xs text-muted-foreground">{p.usageCount} uses</span>;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Create and manage discount campaigns and promo codes
            </p>
          </div>
        </div>
          ...
          <TabsList className="h-8">
            {["all", "active", "draft", "paused", "expired"].map((t) => (
              <TabsTrigger key={t} value={t} className="text-xs capitalize px-3 h-7">
                {t}{" "}
                {tabCounts[t] > 0 && (
                  <span className="ml-1 text-[10px] opacity-60">({tabCounts[t]})</span>
                )}
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          <TableBody>
            ...
            {rows.map((p) => (
              <TableRow key={p.id}>
                ...
                <TableCell>
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${TYPE_COLORS[p.type] ?? ""}`}
                  >
                ...
                <TableCell>
                  <Badge
                    variant="outline"
                    className={`text-[10px] capitalize ${STATUS_COLORS[p.status] ?? ""}`}
                  >
                    {p.status}
                  </Badge>
                </TableCell>
                    ...
                    <div className="flex gap-1">
                      {p.status === "active" && (
                        <Button
                          ...
                          title="Pause"
                          onClick={() => patchStatus.mutate({ id: p.id, status: "paused" })}
                        >
                      ...
                      {p.status === "paused" && (
                        <Button
                          ...
                          title="Resume"
                          onClick={() => patchStatus.mutate({ id: p.id, status: "active" })}
                        >
      ...
      <Dialog open={!!editPromo} onOpenChange={(open) => !open && setEditPromo(null)}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() =>
                editPromo && updatePromo.mutate({ id: editPromo.id, body: buildBody() })
              }
