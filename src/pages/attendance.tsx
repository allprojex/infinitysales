...
export default function Attendance() {
  ...
  const createRec = useMutation({
    mutationFn: (body: object) =>
      customFetch("/api/attendance", { method: "POST", body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const updateRec = useMutation({
    mutationFn: ({ id, body }: { id: string; body: object }) =>
      customFetch(`/api/attendance/${id}`, { method: "PUT", body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const filtered = search
    ? rows.filter((r) => r.employeeName?.toLowerCase().includes(search.toLowerCase()))
    : rows;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Daily employee clock-in and clock-out records
            </p>
          </div>
        </div>
            ...
            {filtered.map((r) => (
              <TableRow key={r.id}>
                ...
                <TableCell className="text-sm text-muted-foreground">
                  {calcHours(r.clockIn, r.clockOut)}
                </TableCell>
                <TableCell>
                  <Badge
                    variant="outline"
                    className={`text-[10px] capitalize ${STATUS_COLORS[r.status] ?? ""}`}
                  >
                    {r.status.replace("_", " ")}
                  </Badge>
                </TableCell>
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={createRec.isPending || !form.employeeId || !form.date}
            >
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => deleteId !== null && deleteRec.mutate(deleteId)}
