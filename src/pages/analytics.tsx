        ...
        <div className="flex items-start justify-between">
          <div className="space-y-1 flex-1 min-w-0">
            ...
            {trendLabel && trend !== "neutral" && (
              <div
                className={`flex items-center gap-1 text-xs font-medium ${trend === "up" ? "text-green-600" : "text-red-500"}`}
              >
                {trend === "up" ? (
          ...
          <div
            className={`h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0`}
          >
...
export default function Analytics() {
  ...
  const { data: stockData } = useListProducts(
    { limit: 100 },
    {
  ...
  const { data: categoryData } = useQuery({
    ...
    queryFn: async () => {
      const d = await customFetch<{
        data: Array<{ category: string | null; totalRevenue: number; totalUnits: number }>;
      }>("/api/reports/top-products?limit=100");
      const cats: Record<string, number> = {};
      d?.data?.forEach((p) => {
        const c = p.category || "Uncategorised";
        cats[c] = (cats[c] || 0) + (p.totalRevenue || 0);
      });
  ...
  const revenueChartData =
    revenue?.map((d: { month: string; revenue: number; sales: number }) => ({
      month: d.month,
  ...
  const topProductsData =
    topProducts?.slice(0, 8).map((p: { name: string; revenue: number; unitsSold: number }) => ({
  ...
  const topCustomersData =
    topCustomers?.map((c: { name: string; totalSpend: number; totalOrders: number }) => ({
      name: c.name.split(" ")[0],
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Full business intelligence — revenue, inventory, customers, and more.
        </p>
      </div>
      ...
      <Tabs defaultValue="overview">
        <TabsList className="flex flex-wrap h-auto gap-1 bg-muted/60 p-1 rounded-xl mb-2">
          ...
          <TabsTrigger value="sales" className="rounded-lg gap-1.5 text-xs">
            <TrendingUp className="h-3.5 w-3.5" />
            Sales
          </TabsTrigger>
          <TabsTrigger value="products" className="rounded-lg gap-1.5 text-xs">
            <Package className="h-3.5 w-3.5" />
            Products
          </TabsTrigger>
          <TabsTrigger value="customers" className="rounded-lg gap-1.5 text-xs">
            <Users className="h-3.5 w-3.5" />
            Customers
          </TabsTrigger>
          <TabsTrigger value="inventory" className="rounded-lg gap-1.5 text-xs">
          ...
          <div className="grid lg:grid-cols-3 gap-4">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-xs">Top categories by revenue</CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <TabsContent value="sales" className="space-y-4">
          <div className="grid sm:grid-cols-3 gap-4">
            <StatCard
              ...
              value={GHS(revenueChartData.at(-1)?.revenue ?? 0)}
              icon={Banknote}
              sub="This month"
            />
            <StatCard
              label="Monthly Orders"
              value={String(revenueChartData.at(-1)?.orders ?? 0)}
              icon={ShoppingCart}
              sub="This month"
            />
            <StatCard
              ...
              value={GHS(
                revenueChartData.length > 0
                  ? revenueChartData.reduce((s, d) => s + d.revenue, 0) / revenueChartData.length
                  : 0,
              )}
              icon={TrendingUp}
            />
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-3">
              <div>
                <CardTitle className="text-sm font-medium">Revenue & Orders Over Time</CardTitle>
              </div>
              <Select value={revMonths} onValueChange={setRevMonths}>
                <SelectTrigger className="w-24 h-8 text-xs rounded-full">
                  <SelectValue />
        ...
        <TabsContent value="products" className="space-y-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium">Units Sold — Top 10 Products</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
        ...
        <TabsContent value="customers" className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <StatCard
              ...
              value={GHS(Number(topCustomers?.[0]?.totalSpend ?? 0))}
              icon={Award}
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {topCustomers?.map(
                    (c: { name: string; totalSpend: number; totalOrders: number }, i: number) => (
                      <div key={i} className="flex items-center gap-3">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium">Customer Spend Distribution</CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={250}>
        ...
        <TabsContent value="inventory" className="space-y-4">
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <StatCard
              ...
              trendLabel={lowStock > 0 ? "Needs attention" : undefined}
            />
            <StatCard
              label="Out of Stock"
              ...
              trendLabel={outOfStock > 0 ? "Urgent" : undefined}
            />
          </div>
          <div className="grid lg:grid-cols-2 gap-4">
            <Card>
                  ...
                  {allStock
                    ...
                    .map((p) => (
                      <div key={p.id} className="flex items-center gap-3">
                        <div
                          className={`h-2 w-2 rounded-full flex-shrink-0 ${p.stock === 0 ? "bg-red-500" : "bg-amber-500"}`}
                        />
                        <p className="flex-1 text-sm truncate">{p.name}</p>
                        <Badge
                          variant="outline"
                          className={`text-[10px] ${p.stock === 0 ? "border-red-300 text-red-600" : "border-amber-300 text-amber-600"}`}
                        >
                      ...
                      <div key={p.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span className="text-muted-foreground">{GHS(value)}</span>
                          </div>
