...
import { Search, Plus, ShoppingBag, MoreVertical, Trash2, Loader2, Check, Package, Upload } from "lucide-react";
...
type PO = {
  id: number; poNumber: string; supplierName: string; status: string;
  subtotal: string; total: string; notes: string | null;
  expectedDate: string | null; receivedDate: string | null; createdAt: string;
  items?: POItem[];
};
...
function statusColor(s: string) {
  switch (s) {
    ...
    case "ordered": return "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300";
    case "received": return "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300";
...
function usePOs(status: string, search: string) {
  return useQuery<{ data: PO[]; total: number; page: number; limit: number }>({
    ...
    queryFn: () => customFetch(`/api/purchase-orders?limit=50${status !== "all" ? `&status=${status}` : ""}${search ? `&search=${encodeURIComponent(search)}` : ""}`),
  });
}
...
function CreatePODialog({ onCreated }: { onCreated: () => void }) {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { toast({ title: "Purchase order created" }); setOpen(false); reset(); onCreated(); },
  ...
  return (
    <Dialog open={open} onOpenChange={o => { setOpen(o); if (!o) reset(); }}>
      <DialogTrigger asChild><Button className="rounded-full gap-2"><Plus className="h-4 w-4" />New Purchase Order</Button></DialogTrigger>
      <DialogContent className="sm:max-w-[620px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 pt-2">
              ...
              <div className="relative">
                ...
                {supplierPickerOpen && suppliersData && (
                  <div className="absolute z-50 left-0 right-0 mt-1 border rounded-xl overflow-hidden bg-card shadow-md max-h-44 overflow-y-auto">
                    {suppliersData.data.length ? [...suppliersData.data].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: "base" })).map(s => (
          ...
          {lineItems.length > 0 && (
            <div className="space-y-2">
              <div className="grid grid-cols-12 text-xs font-medium text-muted-foreground px-1">
                <div className="col-span-5">Product</div><div className="col-span-2 text-center">Qty</div><div className="col-span-3 text-center">Unit Cost (₵)</div><div className="col-span-2 text-right">Total</div>
              </div>
              {lineItems.map((item, idx) => (
                <div key={item.productId} className="grid grid-cols-12 items-center gap-1 bg-muted/30 rounded-xl px-2 py-1.5">
                  <div className="col-span-5 text-sm font-medium truncate">{item.productName}</div>
                  <div className="col-span-2 flex justify-center">
                    <Input type="number" min={1} name={`items[${idx}].quantity`} value={item.quantity} onChange={e => updateItem(idx, "quantity", Number(e.target.value))} className="h-7 w-14 text-center rounded-lg text-xs p-1" />
                  </div>
                  <div className="col-span-3 flex justify-center">
                    <Input type="number" min={0} step="0.01" name={`items[${idx}].unitCost`} value={item.unitCost} onChange={e => updateItem(idx, "unitCost", Number(e.target.value))} className="h-7 w-24 text-center rounded-lg text-xs p-1" />
                  </div>
                  <div className="col-span-2 flex items-center justify-end gap-1">
                    <span className="text-xs font-medium">{GHS(item.quantity * item.unitCost)}</span>
                    <button onClick={() => setLineItems(l => l.filter((_, i) => i !== idx))} className="text-muted-foreground hover:text-destructive ml-1"><CloseIcon className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
              ))}
          ...
          <div><label className="text-sm font-medium block mb-1">Notes</label>
            <Textarea id="po-notes" name="notes" placeholder="Notes for this PO…" value={notes} onChange={e => setNotes(e.target.value)} className="rounded-[20px] resize-none" rows={2} />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" className="rounded-full" onClick={() => setOpen(false)}>Cancel</Button>
            <Button className="rounded-full" disabled={!supplierName.trim() || createMut.isPending} onClick={() => createMut.mutate({ supplierName, expectedDate: expectedDate || null, notes: notes || null, items: lineItems })}>
...
export default function Purchases() {
  ...
  const invalidate = () => { queryClient.invalidateQueries({ queryKey: ["purchase-orders"] }); queryClient.invalidateQueries({ queryKey: getListProductsQueryKey() }); };

  const updateStatus = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) => customFetch(`/api/purchase-orders/${id}`, { method: "PUT", body: JSON.stringify({ status }), headers: { "Content-Type": "application/json" } }),
    onSuccess: () => { toast({ title: "Status updated" }); invalidate(); },
    onError: (e) => toast({ variant: "destructive", title: "Error", description: e.message }),
  });

  const receivePO = useMutation({
    mutationFn: (id: number) => customFetch(`/api/purchase-orders/${id}/receive`, { method: "POST" }),
    onSuccess: () => { toast({ title: "Stock updated — PO received!" }); setViewingId(null); invalidate(); },
    onError: (e) => toast({ variant: "destructive", title: "Error", description: e.message }),
  });

  const deletePO = useMutation({
    mutationFn: (id: number) => customFetch(`/api/purchase-orders/${id}`, { method: "DELETE" }),
    onSuccess: () => { toast({ title: "Purchase order deleted" }); setDeletingId(null); invalidate(); },
    onError: (e) => toast({ variant: "destructive", title: "Error", description: e.message }),
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Create and track purchase orders from suppliers — receiving updates stock automatically.</p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative w-[220px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input id="po-search" name="search" placeholder="Search supplier…" className="pl-9 rounded-full" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
          ...
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {poDetail.status !== "received" && poDetail.status !== "cancelled" && (
                <Button size="sm" className="rounded-full gap-2 bg-emerald-600 hover:bg-emerald-700"
                  onClick={() => receivePO.mutate(poDetail.id)} disabled={receivePO.isPending}>
                  {receivePO.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                  Receive & Update Stock
                </Button>
              )}
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => setViewingId(null)}>
          ...
          <CardContent>
            <div className="space-y-2">
              {poDetail.items?.map(item => (
                <div key={item.productId} className="flex items-center justify-between p-2.5 bg-muted/30 rounded-xl text-sm">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-4 text-muted-foreground">
                    ...
                    <span className="font-bold text-foreground">{GHS(item.quantity * item.unitCost)}</span>
                  </div>
                </div>
              ))}
      ...
      <AlertDialog open={deletingId !== null} onOpenChange={o => { if (!o) setDeletingId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete purchase order?</AlertDialogTitle><AlertDialogDescription>This cannot be undone.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground" onClick={() => deletingId && deletePO.mutate(deletingId)} disabled={deletePO.isPending}>
      ...
      {isLoading ? (
        ...
        <div className="bg-card rounded-3xl border border-dashed border-muted-foreground/20 p-12 text-center">
          ...
          <p className="text-muted-foreground mb-6">Create your first PO to restock from suppliers and auto-update inventory on receipt.</p>
        </div>
      ) : (
        <div className="bg-card rounded-2xl border overflow-x-auto shadow-sm">
          <div className="grid grid-cols-6 px-4 py-2 border-b text-xs font-medium text-muted-foreground bg-muted/30 min-w-[560px]">
            <div>PO Number</div><div className="col-span-2">Supplier</div><div>Status</div><div>Expected</div><div className="text-right">Total</div>
          </div>
          {pos.map(po => (
            <div key={po.id} className={`grid grid-cols-6 px-4 py-3 border-b last:border-0 items-center text-sm cursor-pointer hover:bg-muted/20 transition-colors min-w-[560px] ${viewingId === po.id ? "bg-primary/5" : ""}`} onClick={() => setViewingId(viewingId === po.id ? null : po.id)}>
              ...
              <div><Badge className={`text-[10px] border-0 ${statusColor(po.status)}`}>{po.status}</Badge></div>
              <div className="text-muted-foreground text-xs">{po.expectedDate ? new Date(po.expectedDate).toLocaleDateString("en-GH") : "—"}</div>
              <div className="flex items-center justify-end gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    {po.status === "draft" && <DropdownMenuItem onClick={e => { e.stopPropagation(); updateStatus.mutate({ id: po.id, status: "ordered" }); }}>Mark as Ordered</DropdownMenuItem>}
                    {po.status !== "received" && po.status !== "cancelled" && (
                      <DropdownMenuItem onClick={e => { e.stopPropagation(); receivePO.mutate(po.id); }}>
                        <Check className="h-4 w-4 mr-2 text-emerald-600" />Receive & Update Stock
                      </DropdownMenuItem>
                    )}
                    {po.status !== "received" && (
                      <DropdownMenuItem onClick={e => { e.stopPropagation(); updateStatus.mutate({ id: po.id, status: "cancelled" }); }}>Cancel</DropdownMenuItem>
                    )}
                    {po.status !== "received" && <><DropdownMenuSeparator /><DropdownMenuItem className="text-destructive focus:text-destructive" onClick={e => { e.stopPropagation(); setDeletingId(po.id); }}><Trash2 className="h-4 w-4 mr-2" />Delete</DropdownMenuItem></>}
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </div>
      ...
      <p className="text-xs text-muted-foreground text-right">{data?.total ?? 0} purchase order{(data?.total ?? 0) !== 1 ? "s" : ""} total</p>
    </div>
  );
}
