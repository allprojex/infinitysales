...
interface LeaveResp { data: LeaveRequest[]; total: number; }
...
const EMPTY_FORM = { employeeId: "", type: "annual", startDate: "", endDate: "", days: "1", reason: "" };
...
export default function Leave() {
  ...
  const createReq = useMutation({
    mutationFn: (body: object) => customFetch("/api/leave", { method: "POST", body: JSON.stringify(body) }),
    onSuccess: () => { toast({ title: "Leave request created" }); qc.invalidateQueries({ queryKey: ["leave"] }); setShowForm(false); setForm(EMPTY_FORM); },
  ...
  const approve = (r: LeaveRequest) => updateReq.mutate({ id: r.id, body: { ...r, status: "approved", approvedBy: user?.name } });
  const reject = (r: LeaveRequest) => updateReq.mutate({ id: r.id, body: { ...r, status: "rejected", approvedBy: user?.name } });

  const openEdit = (r: LeaveRequest) => {
    setEditReq(r);
    setForm({ employeeId: String(r.employeeId), type: r.type, startDate: r.startDate, endDate: r.endDate, days: String(r.days), reason: r.reason ?? "" });
  };
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div>
          ...
          <Input className="h-8 text-xs" type="number" min="1" value={form.days} onChange={e => setForm(f => ({ ...f, days: e.target.value }))} />
        </div>
      </div>
      <div>
        <label className="text-xs text-muted-foreground font-medium mb-1 block">Reason</label>
        <Textarea className="text-xs" rows={2} value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))} placeholder="Reason for leave…" />
      </div>
    </div>
  );
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Track and approve employee leave requests</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => refetch()}><RefreshCw className="h-3.5 w-3.5" /></Button>
          {isAdmin && <Button size="sm" onClick={() => { setShowForm(true); setForm(EMPTY_FORM); }} className="gap-1.5"><Plus className="h-3.5 w-3.5" />New Request</Button>}
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <span className="text-xs text-muted-foreground flex items-center">{rows.length} requests</span>
      </div>
      ...
      <Card>
        <Table>
          ...
          <TableBody>
            {isLoading && <TableRow><TableCell colSpan={6} className="text-center py-10"><RefreshCw className="h-4 w-4 animate-spin mx-auto text-muted-foreground" /></TableCell></TableRow>}
            {!isLoading && rows.length === 0 && <TableRow><TableCell colSpan={6} className="text-center py-10 text-muted-foreground">No leave requests</TableCell></TableRow>}
            {rows.map(r => {
              ...
              return (
                <TableRow key={r.id}>
                  ...
                  <TableCell><span className="capitalize text-sm">{r.type}</span></TableCell>
                  <TableCell className="text-xs">{r.startDate} → {r.endDate}</TableCell>
                  <TableCell className="text-center font-medium">{r.days}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className={`text-[10px] gap-1 ${STATUS_COLORS[r.status] ?? ""}`}>
                  ...
                  {isAdmin && (
                    <TableCell>
                      <div className="flex gap-1 flex-wrap">
                        {r.status === "pending" && <>
                          <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px] text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10" onClick={() => approve(r)}>Approve</Button>
                          <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px] text-red-400 hover:text-red-300 hover:bg-red-500/10" onClick={() => reject(r)}>Reject</Button>
                        </>}
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => openEdit(r)}><Pencil className="h-3 w-3" /></Button>
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:text-destructive" onClick={() => setDeleteId(r.id)}><Trash2 className="h-3 w-3" /></Button>
                      </div>
                    </TableCell>
                  )}
                </TableRow>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={open => !open && setDeleteId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deleteId !== null && deleteReq.mutate(deleteId)} className="bg-destructive hover:bg-destructive/90">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
