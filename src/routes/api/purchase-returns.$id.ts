  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const { data, error } = await sb.from("purchase_returns").select("*")
      ...
      PATCH: async ({ request, params }) => {
        ...
        const { data, error } = await sb.from("purchase_returns").update(apiToRow(body) as any)
      ...
      DELETE: async ({ request, params }) => {
        const auth = await requireUser(request); if (auth.response) return auth.response;
        const { error } = await sb.from("purchase_returns").delete()
