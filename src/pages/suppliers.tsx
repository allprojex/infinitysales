...
function useSuppliers(search: string) {
  return useQuery<{ data: Supplier[]; total: number; page: number; limit: number }>({
    ...
    queryFn: () =>
      customFetch(
        `/api/suppliers?limit=50${search ? `&search=${encodeURIComponent(search)}` : ""}`,
      ),
  });
}
    ...
    <div className="space-y-4 pt-2">
      ...
      <div>
        ...
        <Input
          id="supplier-address"
          name="address"
          placeholder="45 Liberation Road, Accra"
          value={address}
...
export default function Suppliers() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (e: Error) =>
      toast({ variant: "destructive", title: "Failed to update supplier", description: e.message }),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e: Error) =>
      toast({ variant: "destructive", title: "Failed to delete supplier", description: e.message }),
  });
